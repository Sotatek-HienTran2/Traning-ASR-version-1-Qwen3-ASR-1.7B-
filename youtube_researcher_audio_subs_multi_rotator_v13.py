...
# ================= FILTER CONFIG =================
FILTER_MIN_DURATION = 300    # 30 phút
...
class YouTubeResearcher:
    ...
    def _apply_auth_skip(ydl_opts: dict) -> dict:
        ...
        # v13.3 FIX: Dùng multi-client ["web_embedded", "android_vr", "mweb"]
        # để fallback khi 1 client bị block (vd: tv + cookies → 403).
        # - web_embedded: ổn định, cần n challenge (có EJS + node)
        # - android_vr: nhanh, ít bị rate-limit, không cần n challenge
        # - mweb: mobile web, fallback tốt khi embedded bị chặn
        # Tránh: tv (403+cookies), web (only images), ios (cần PO Token),
        #        android (SABR-only), web_safari (only images)
        if "player_client" not in yt_args:
            yt_args["player_client"] = ["web_embedded", "android_vr", "mweb"]
        if "js_runtimes" not in ydl_opts:
            # v13.1: Dùng shutil.which("node") thay vì hardcode path
            import shutil
    ...
    def _build_audio_index(audio_root, min_size_bytes: int = 50 * 1024) -> dict:
        ...
        v13-FIX: Thêm scan root flat (giống v15/v16) để nhận WAV files nằm
        phẳng ngay tại `audio/` (không có subdir timestamp). Subdir mới
        overwrite root cũ khi trùng key (ưu tiên bản mới nhất).
        ...
        # v13-FIX: scan root flat trước, subdir mới overwrite root cũ khi trùng key
    ...
    def _build_json_index(transcriptions_root, min_size_bytes: int = 100) -> dict:
        ...
        v13-FIX: Thêm scan root flat (giống v15/v16) để nhận JSON files nằm
        phẳng ngay tại `transcriptions/` (không có subdir timestamp). Subdir
        mới overwrite root cũ khi trùng key (ưu tiên bản mới nhất).
    ...
    def _partition_videos_for_pipeline(
        ...
        for video in self._filtered_videos:
            ...
            json_path = None
            if audio_path:
                # v13-FIX: Bỏ check `cand.stem == expected_json_stem` (luôn sai vì
                # json_index đã strip hậu tố `_transcription`). Chỉ cần `if cand`.
                # Thêm key combo `{target_name}_{video.video_id}` để match thêm trường
                # hợp audio file được rename kiểu title_id.wav.
                for key in [
                    audio_path.stem,
                    video.video_id,
                    target_name,
        ...
        while fetched < max_results:
            ...
            if items_this_page > 0 and page_count % 5 == 0:
                ...
                print(f"  [Phase 1] page {page_count}: fetched {fetched} videos "
                      f"({elapsed:.0f}s)")
            # v13-FIX: Dừng sớm khi response rỗng (items_this_page == 0)
            # thay vì chờ hết nextPageToken. Tránh tốn ~13s cho mỗi page trống.
            if items_this_page == 0:
                elapsed = time.time() - phase1_start
                print(f"  [Phase 1] STOP: page {page_count} returned 0 items, "
                      f"đã fetch hết {fetched} videos ({elapsed:.0f}s)")
                break
            if not page_token:
                break
    ...
    def _get_youtube_transcript_via_ytdlp(self, video_id: str,
        ...
        if info is None:
            ...
            try:
                ...
                # v13-FIX: Nếu DIRECT (proxy_url=None) bị block → set _direct_blocked
                # để retry sau KHÔNG dùng DIRECT nữa, mà dùng transcript_rotator.
                if not proxy_url:
                    err_lower = err_str.lower()
                    is_blocked = any(kw in err_lower for kw in [
                        "sign in to confirm", "not a bot", "captcha",
                        "http error 403", "forbidden", "rate-limit",
                    ])
                    if is_blocked:
                        print(f"  [ytdlp-subs] DIRECT bị block → set _direct_blocked=True")
                        self._direct_blocked = True
        ...
        try:
            ...
            # v13-FIX: Nếu DIRECT (proxy_url=None) bị block trong download sub
            # → set _direct_blocked để retry sau dùng transcript_rotator.
            if not proxy_url:
                err_lower = err_str.lower()
                is_blocked = any(kw in err_lower for kw in [
                    "sign in to confirm", "not a bot", "captcha",
                ...
                if is_blocked:
                    print(f"  [ytdlp-subs] DIRECT bị block (download) → set _direct_blocked=True")
                    self._direct_blocked = True
    ...
    def transcribe_with_youtube(self, video_id: str, audio_path: Path = None,
        ...
        result = None
        # v13-FIX: range(1) → range(3) để retry thật sự 3 lần (trước đây chỉ
        # chạy 1 attempt, không retry → DIRECT bị block thì xong luôn).
        for a in range(3):
            if a == 0:
