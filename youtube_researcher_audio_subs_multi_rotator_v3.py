...
class YouTubeResearcher:
    ...
    def _partition_videos_for_pipeline(
        ...
        Args:
            audio_root: thư mục gốc audio (vd: output_dir/audio/), sẽ scan
                tất cả subfolder {timestamp}/ bên trong.
            transcriptions_root: thư mục gốc transcriptions/ tương tự.
            skip_existing: nếu False (--force-retranscribe) → tất cả video
                vào Bucket C để xử lý từ đầu.
        ...
        # Pre-build 2 index (1 disk scan mỗi loại)
        ...
        for video in self._filtered_videos:
            ...
            # Lookup audio (3 pattern giống find_existing_audio)
            audio_path = (
            ...
            # Lookup json (chỉ chấp nhận json match với audio_path)
            json_path = None
            if audio_path:
...
def _process_videos_pipeline(self, output_dir, run_timestamp="",
    ...
    Refactor v3.1: 3 bucket A/B/C (giống v2) — code sạch, dễ debug, dễ maintain.
      - Bucket A: có CẢ audio + JSON matching → SKIP nhanh (0 I/O)
      - Bucket B: có audio (run cũ), thiếu JSON → chỉ transcribe
      - Bucket C: chưa có audio → full pipeline (download + transcribe + save)
    ...
    for i, (video, target_name, target_filename) in enumerate(bucket_c, 1):
        ...
        try:
            ...
            for dl_attempt in range(1, dl_retries + 1):
                dl_proxy = None
                use_vpn = True
                # === v3.1: Tracking download speed (fixed window, skip 5s handshake) ===
                # Bỏ qua 5s đầu (handshake YouTube). Sau đó chia thành các
                # window cố định 5s, MỖI window đánh giá tốc độ 1 LẦN:
                #   - window 1: (5, 10]s → đo bytes trong khoảng / 5s
                #   - window 2: (10, 15]s → đo bytes trong khoảng / 5s
                #   - ...
                # Tại cuối mỗi window: nếu speed < min_speed → rotate IP ngay.
                SKIP_HANDSHAKE_S = 5.0
                WINDOW_S = 5.0
                MIN_BYTES_FOR_SPEED = 100 * 1024
                MIN_SPEED_MBPS = 1.0
                ...
                dl_state = {
                    ...
                    "rotate_triggered": False,
                }

                def _audio_progress_hook(d):
                    """Check fixed window speed, rotate IP nếu chậm.

                    Mỗi chunk progress:
                      1. elapsed_s = now - t_start
                      2. Nếu elapsed_s > 5s: window_idx = floor((elapsed_s - 5) / 5)
                      3. Nếu window_idx > current_window_idx (window mới):
                         - speed = (bytes_now - bytes_at_window_start) / 5s
                         - Nếu speed < 1 MB/s → rotate IP ngay
                         - Cập nhật current_window_idx, bytes_at_window_start
                    """
                    if d.get("status") != "downloading":
                        return
                    now = time.time()
                    bytes_now = int(d.get("downloaded_bytes") or 0)
                    elapsed_s = now - dl_state["t_start"]

                    # Bỏ qua 5s handshake đầu
                    if elapsed_s <= SKIP_HANDSHAKE_S:
                        return

                    # Tính window index hiện tại (0 = (5,10], 1 = (10,15], ...)
                    window_idx = int((elapsed_s - SKIP_HANDSHAKE_S) / WINDOW_S)

                    # Đã rotate IP rồi trong attempt này → không đánh giá thêm
                    # (download hiện tại sẽ retry với IP mới ở attempt sau)
                    if dl_state["rotate_triggered"]:
                        return

                    # Vào window MỚI
                    if window_idx > dl_state["current_window_idx"]:
                        if dl_state["current_window_idx"] == -1:
                            dl_state["bytes_at_window_start"] = bytes_now
                        # Tính speed trong window vừa KẾT THÚC
                        bytes_in_window = bytes_now - dl_state["bytes_at_window_start"]
                        if bytes_in_window >= MIN_BYTES_FOR_SPEED:
                            speed_mbps = bytes_in_window / (WINDOW_S * 1024 * 1024)
                            w_idx = dl_state["current_window_idx"]
                            t_start_win = SKIP_HANDSHAKE_S + w_idx * WINDOW_S
                            t_end_win = SKIP_HANDSHAKE_S + (w_idx + 1) * WINDOW_S
                            if speed_mbps < MIN_SPEED_MBPS:
                                # Lấy thông tin IP hiện tại TRƯỚC khi rotate
                                old_ip = None
                                old_server = "?"
                                new_ip = None
                                new_server = "?"
                                if self._audio_rotator is not None:
                                    try:
                                        old_ip = getattr(self._audio_rotator, "_current_ip", None)
                                        cur_idx = getattr(self._audio_rotator, "_current_idx", None)
                                        ovpn_files = getattr(self._audio_rotator, "_ovpn_files", [])
                                        if cur_idx is not None and cur_idx < len(ovpn_files):
                                            old_server = ovpn_files[cur_idx].name
                                    except Exception:
                                        pass
                                print(f"    [audio-dl] SLOW window({t_start_win:.0f},{t_end_win:.0f}] "
                                      f"{bytes_in_window//1024}KB in {WINDOW_S:.0f}s "
                                      f"= {speed_mbps:.2f} MB/s (< {MIN_SPEED_MBPS} MB/s) "
                                      f"→ CANCEL + ROTATE IP (current: {old_server} / {old_ip})",
                                      flush=True)
                                # CANCEL download hiện tại → outer try/except catch → retry với IP mới
                                if self._audio_rotator is not None:
                                    try:
                                        self._audio_rotator.next()
                                        new_ip = getattr(self._audio_rotator, "_current_ip", None)
                                        cur_idx = getattr(self._audio_rotator, "_current_idx", None)
                                        ovpn_files = getattr(self._audio_rotator, "_ovpn_files", [])
                                        if cur_idx is not None and cur_idx < len(ovpn_files):
                                            new_server = ovpn_files[cur_idx].name
                                        print(f"      [audio-dl] 🔄 ROTATED to FAKE IP: "
                                              f"server={new_server}, ip={new_ip} "
                                              f"(was: server={old_server}, ip={old_ip})",
                                              flush=True)
                                    except Exception as e:
                                        print(f"      [audio-dl] Rotate error: {e}", flush=True)
                                dl_state["rotate_triggered"] = True
                                # Raise exception để CANCEL download hiện tại
                                # Outer try/except sẽ catch → retry attempt mới với IP mới
                                raise Exception(
                                    f"audio-slow-rotate: speed={speed_mbps:.2f} MB/s "
                                    f"< {MIN_SPEED_MBPS} MB/s in window({t_start_win:.0f},{t_end_win:.0f}] "
                                    f"→ switched to {new_server}/{new_ip}")
                            else:
                                print(f"    [audio-dl] OK window({t_start_win:.0f},{t_end_win:.0f}] "
                                      f"{bytes_in_window//1024}KB in {WINDOW_S:.0f}s "
                                      f"= {speed_mbps:.2f} MB/s (>= {MIN_SPEED_MBPS} MB/s)",
                                      flush=True)
                        # Sang window mới
                        dl_state["current_window_idx"] = window_idx
                        dl_state["bytes_at_window_start"] = bytes_now
                ...
                try:
                    ...
                    # === v3.2: Catch audio-slow-rotate exception từ progress hook ===
                    # Khi IP chậm, hook đã cancel download + rotate IP.
                    # Retry ngay attempt tiếp theo với IP mới.
                    if "audio-slow-rotate" in str(dl_err) and dl_attempt < dl_retries:
                        print(f"    [audio-dl] attempt {dl_attempt}/{dl_retries} "
                              f"CANCELLED (IP slow) → retry ngay với IP mới", flush=True)
                        time.sleep(2)  # Brief pause để IP rotator ổn định
                        continue
