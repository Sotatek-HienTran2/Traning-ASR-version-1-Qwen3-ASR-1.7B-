# =============================================================================
# v12: Smart timeout/IP detection — clone v11 + mid-download slow-speed rotation.
# =============================================================================
# Khác biệt so với v11 (v11_smart_downloader.py):
#   - Thêm class `MidDownloadRotate` (exception) để progress hook có thể báo
#     "đang tải nhưng speed quá chậm trong cửa sổ N giây" → download_with_smart_retry
#     catch → force_rotate NGAY (không đợi DownloadError).
#   - Thêm tham số `slow_speed_kbps` (KB/s) và `slow_window_seconds` (s) vào
#     download_with_smart_retry. Khi progress_hook phát hiện rolling avg < ngưỡng
...
# === v12: Exception riêng cho "đang tải nhưng speed chậm → rotate" ===
class MidDownloadRotate(Exception):
    ...
    def __init__(self, avg_kbps: float, window_seconds: float,
        ...
        super().__init__(
            f"MidDownloadRotate: avg_speed={avg_kbps:.1f} KB/s "
    ...
    def download_with_smart_retry(
        ...
        for attempt in range(1, attempts + 1):
            ...
            try:
                ...
                # === v12: Đang tải giữa chừng nhưng avg speed < ngưỡng ===
                # KHÔNG đợi DownloadError, force_rotate NGAY và retry.
                last_err = str(slow_exc)
                elapsed = time.time() - t_start
                log.warning(
                    "[smart-dl] MID-DOWNLOAD SLOW attempt=%d avg=%.1f KB/s "
                    "window=%.1fs bytes_dl=%d elapsed=%.1fs",
                    attempt, slow_exc.avg_kbps, slow_exc.window_seconds,
                    slow_exc.bytes_dl, slow_exc.elapsed_s,
                )
                print(
                    f"  [v12-smart] 🐌 MID-DOWNLOAD SLOW attempt={attempt} "
                    f"avg={slow_exc.avg_kbps:.1f} KB/s over "
                    f"{slow_exc.window_seconds:.1f}s "
                    f"(threshold={slow_speed_kbps} KB/s, "
                    f"bytes_dl={slow_exc.bytes_dl//1024}KB, "
                    f"elapsed={slow_exc.elapsed_s:.1f}s) → FORCE ROTATE",
                    flush=True,
                )
                ...
                if cap_reached:
                    print(
                        f"  [v12-smart] ⛔ Reached max_rotate_per_video="
                    ...
                    rotate_history.append({
                        ...
                        "avg_kbps": slow_exc.avg_kbps,
                        "window_s": slow_exc.window_seconds,
                        "elapsed_s": elapsed,
                    })
                    break  # thoát vòng for, trả về ok=False
                ...
                # Sleep ngắn để VPN settle + DNS re-resolve
