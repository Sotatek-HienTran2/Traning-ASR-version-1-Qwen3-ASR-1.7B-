#!/usr/bin/env python3
"""
YouTube Researcher -- AUDIO + YOUTUBE SUBS (MULTI-ROTATOR) -- v14
================================================================================

v14 = v13 + TỐI ƯU LẤY VIETSUB (giữ nguyên toàn bộ logic audio/download
       của v13; chỉ sửa phần nhận diện + retry sub để tăng tỷ lệ lấy được
       sub Tiếng Việt, đặc biệt với video Việt Nam có auto-generated subs).

VẤN ĐỀ CỦA V13 về VIETSUB (Qua test thực tế ~87% miss sub VN):
  1) `_find_vi_lang()` chỉ trả về 1 key ĐẦU TIÊN có `startswith("vi")`.
     → Bỏ qua scoring/priority giữa các biến thể:
        "vi-orig" (auto-gen gốc, phổ biến nhất cho video VN)
        > "vi-VN" > "vi-VN-x-taglish" > "vi" (manual) > "vi" (auto)
     → Khi "vi" manual rỗng / lỗi URL, code KHÔNG fallback xuống "vi-orig".

  2) `transcribe_with_youtube()` có `for a in range(1)` ở line 3312 —
     chỉ chạy ĐÚNG 1 LẦN thay vì 3 lần như comment nói → retry bị vô hiệu.

  3) Marker `.no_transcript` đóng băng vĩnh viễn → 19,658 video đã bị
     mark sai do rate-limit tạm thời → KHÔNG bao giờ được retry.

  4) Khi thấy "no Vi sub" → return None ngay → KHÔNG log đầy đủ các
     languages CÓ (để debug), KHÔNG thử các biến thể khác (vi-orig,...).

V14 FIX (4 lớp):
  1) **VIETSUB SCORING ENGINE** (`_score_vi_subs()` + `_find_vi_lang_v14()`):
     - Trả về LIST các key VI match, SORT theo priority score.
     - Priority order (cao → thấp):
         vi-orig (auto-gen gốc) > vi-VN (auto VN region) >
         vi-VN-x-* (taglish) > vi (manual) > vi (auto) > vi-*
     - Cho phép user chỉ định --vi-sub-priority qua CLI.
     - Log đầy đủ: TẤT CẢ languages có trong subtitles/automatic_captions,
       kèm key được chọn + score + lý do.

  2) **RETRY ĐÚNG 3 LẦN** (`transcribe_with_youtube()`):
     - Sửa bug `for a in range(1)` → `range(3)`.
     - Mỗi retry: force_rotate transcript_rotator trước khi lấy proxy
       (tránh dùng server đã bị Google flag ở attempt trước).
     - Backoff exponential: 3s, 6s, 12s.

  3) **MARKER TTL** (`_has_no_transcript_marker()`):
     - Mặc định: bỏ qua marker cũ (>7 ngày) → cho phép retry video đã
       mark sai do rate-limit tạm thời.
     - Flag `--respect-no-transcript-marker`: giữ hành vi v13 (skip vĩnh viễn).
     - Flag `--no-marker-ttl-days N`: chỉnh TTL (default 7).
     - KHÔNG ghi marker mới cho Bucket C nếu user chạy `--retry-no-transcript`
       (tránh đè marker retry).

  4) **DEBUG LOG**:
     - Log TẤT CẢ key có trong subtitles + automatic_captions.
     - Log SCORE + key được chọn + lý do.
     - Log TỔNG KẾT cuối channel: bao nhiêu VN/manual/auto/không có.

GIỮ NGUYÊN TỪ V13 (KHÔNG SỬA):
  - HTTP500Detector stall fire mỗi 30s.
  - AudioIPController force_real_after_2_fake_fails.
  - 3 VPN rotator riêng biệt (metadata/audio/transcript) per-instance.
  - Per-instance tunnel kill (v11).
  - Multi-bucket pipeline (A/B/C) với skip-existing.
  - Bucket A: SKIP nhanh.
  - KHÔNG XÓA audio đã tải khi re-run (chỉ skip khi --force-redownload).

v14.1 (MỚI so với v14):
  - BỎ cơ chế "tức thời" (speed_mbps = bytes_dl/elapsed_s) trong
    AudioIPController.on_download_complete(). Chỉ dùng _slow_flag (window
    cố định) + ok để quyết định đổi IP.
  - BỎ cụm raise MidDownloadRotate do _slow_flag=True trong _audio_progress_hook.
  - BỎ cụm _v13_speed_samples rolling window + raise MidDownloadRotate
    trong _audio_progress_hook.
  - Cấu hình --audio-slow-* (speed/window/max-rotate) vẫn được truyền vào
    v13_smart_downloader.download_with_smart_retry() làm cap rotate, nhưng
    KHÔNG raise MidDownloadRotate từ hook nữa.

Output: giống v13.

CÁCH DÙNG:
    # Chạy bình thường (transcribe + tải sub VN ưu tiên vi-orig)
    python youtube_researcher_audio_subs_multi_rotator_v14.py \\
        --channels-file ./channels_audio/channels_khoa_hoc.txt \\
        --instance-id inst_v14 --skip-existing

    # Retry video đã mark .no_transcript (bỏ qua marker cũ > 7 ngày)
    python ..._v14.py --channels-file ... --retry-no-transcript

    # Force re-transcribe kể cả video đã có JSON
    python ..._v14.py --channels-file ... --force-retranscribe

    # Giữ hành vi v13 (marker vĩnh viễn)
    python ..._v14.py --channels-file ... --respect-no-transcript-marker

    # Custom priority (vd: ưu tiên vi manual)
    python ..._v14.py --channels-file ... --vi-sub-priority manual_first

    # Xóa tất cả audio cũ và tải lại (KHÔNG khuyến nghị)
    python ..._v14.py --channels-file ... --force-redownload
"""
import json
import os
import re
...
class YouTubeResearcher:
    ...
    def _get_youtube_transcript_via_ytdlp(self, video_id: str,
        ...
        # v13 cũ chỉ tìm key đầu tiên startswith("vi") qua `_find_vi_lang()`
        # → miss khi "vi" manual rỗng hoặc URL lỗi.
        # v14: dùng `_score_vi_subs()` để lấy LIST các key VI match,
        # sort theo priority. Thử TỪNG key theo thứ tự score giảm dần
        # cho đến khi parse được segments thành công.
        ...
        # v13 cũ: chọn 1 key → download → parse. Nếu URL lỗi → return None.
        # v14: thử TỪNG key theo score giảm dần. Nếu key hiện tại download/parse
        # fail (0 segments, timeout, 4xx), thử key tiếp theo. Chỉ return None
        # khi TẤT CẢ key đều fail.
        best_result = None
        best_meta = None  # (source_type, lang_code, sub_format) của key tốt nhất
        tried_keys = []
        ...
        for key_idx, (key, _dict, source_type, score, lang_code) in enumerate(scored):
            ...
            try:
                ...
                if resp.status_code == 429:
                    ...
                    if proxy_url:
                        self._mark_transcript_proxy_failed(proxy_url)
                    # KHÔNG return None — thử key tiếp theo
                    continue
                if resp.status_code == 403:
                    print(f"    [ytdlp-subs] HTTP 403 (forbidden) — try next key")
                    if proxy_url:
                        self._mark_transcript_proxy_failed(proxy_url)
                    continue
            ...
            # SUCCESS cho key này
            # Map source_type: "subtitles"→"manual", "automatic_captions"→"auto"
            # để giữ backward-compat với format source cũ (v13 trả về
            # "yt-dlp-json3-manual"/"yt-dlp-json3-auto").
            ...
            break  # Lấy key tốt nhất có kết quả → DONE
    ...
    def transcribe_with_youtube(self, video_id: str, audio_path: Path = None,
        ...
        # v14: Retry toi da 3 lan voi force_rotate moi khi fail (FIX bug v13).
        # - v13 BUG: `for a in range(1)` ở line 3501 — chỉ chạy 1 lần, comment
        #   nói 3 lần nhưng thực tế chỉ retry 1 lần (vì `break` khi success).
        # - v14 FIX: `range(3)` để chạy đủ 3 attempts.
        # - Attempt 1: proxy bình thường (hoặc IP thật nếu _direct_blocked).
        # - Attempt 2: force_rotate transcript_rotator (tránh server bị Google flag).
        # - Attempt 3: force_rotate tiếp (last chance).
        # - Mỗi attempt có backoff exponential 3s, 6s, 12s.
        result = None
        max_attempts = 3
        for a in range(max_attempts):
            if a == 0:
                ...
                # v4 + v14: FORCE rotate trước mỗi retry (tránh dùng server
                # bị flag ở attempt trước).
                ...
                backoff = 3 * (2 ** (a - 1))
                print(f"    [v14-retry] sleeping {backoff}s before attempt {a+1}")
                time.sleep(backoff)
        ...
        # === v14: FALLBACK youtube-transcript-api ===
        # Sau khi yt-dlp fail hết max_attempts → thử youtube-transcript-api
        # (engine khác, gọi timedtext API của YouTube — thường bypass captcha
        # tốt hơn yt-dlp khi IP/proxy đã bị Google flag).
        # Pattern: tham khảo từ `youtube_researcher_youtube_subs.py` (file cũ).
        # Disable qua CLI flag --no-api-fallback (mặc định BẬT).
        if not result:
            api_fallback_enabled = getattr(self, "_v14_api_fallback_enabled", True)
            if api_fallback_enabled:
                print(f"  [v14-fallback] yt-dlp fail sau {max_attempts} attempts "
                      f"→ thử youtube-transcript-api engine...")
                # Force rotate proxy để lấy IP mới cho api engine
                if self._transcript_rotator:
                    try:
                        self._transcript_rotator.force_rotate(
                ...
                # Try 1 lần với youtube-transcript-api
                result = self._get_youtube_transcript_via_api(
        ...
        if not result:
            print(f"  No YouTube transcript available (yt-dlp fail sau "
                  f"{max_attempts} attempts + api-fallback fail)")
            return None
