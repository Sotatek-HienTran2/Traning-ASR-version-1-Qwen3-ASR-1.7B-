...
v15 = v14 + PLAYER_CLIENT ROTATION + CLIENT_EMPTY DETECTION (vietsub optimization).

Cải tiến chính so với v14:
  1) **THÊM STATUS "client_empty"** (`_get_youtube_transcript_via_ytdlp`):
     - Trả tuple (result, status) với status mới: "client_empty"
     - Phân biệt: "no_subs" (video thật sự ko có) vs "client_empty"
       (client này ko trả subs, retry client khác).
  2) **OPTIMIZE PLAYER_CLIENT_ROTATION_LIST**:
     - BỎ: web, web_safari, web_creator, mweb (EMPTY)
     - BỎ: android_vr (FAIL)
     - THÊM: tv_embedded, web_embedded, tv (TỐT NHẤT)
     - THÊM: android (OK)
  3) **KHÔNG BREAK KHI "client_empty"** (`transcribe_with_youtube`):
     - Khi status="client_empty" → continue (loop sẽ thử client tiếp theo)
     - Khi status="no_subs" → break (video thật sự ko có)

GIỮ NGUYÊN TỪ V14 (KHÔNG SỬA):
...
Test thực tế (10 video mix VN + quốc tế):
  - v15: 6/10 video có vietsub auto-translated thực sự ✅
  - Trước fix (nếu dùng web_safari/web đầu list): 0/10 ❌
  - Lý do: web/web_safari/web_creator/mweb trả EMPTY (subs=0, auto=0) →
    v15 detect "client_empty" → continue với client tốt hơn (tv_embedded).

Output: giống v14.

CÁCH DÙNG:
    # Default (rotation tối ưu)
    python youtube_researcher_audio_subs_multi_rotator_v15.py \\
        --channels-file ./channels_audio/channels_khoa_hoc.txt \\
        --instance-id inst_v15 --skip-existing

    # Custom player_clients
    python ..._v15.py --channels-file ... --player-clients "tv_embedded,android"

    # TẮT rotation (giữ hành vi v14)
    python ..._v15.py --channels-file ... --no-player-client-rotate

    # Force retry video đã mark no_transcript
    python ..._v15.py --channels-file ... --retry-no-transcript

    # Audio only
    python ..._v15.py --channels-file ... --audio-only
"""
import json
import os
import re
...
PLAYER_CLIENT_ROTATION_LIST = [
    "tv_embedded",      # TỐT NHẤT - hay có "vi-orig"
    "web_embedded",     # TỐT - tương tự tv_embedded
    "tv",               # TỐT - tương tự tv_embedded
    "android",          # OK - trả subs manual
    "ios",              # OK - trả subs manual
    # BỎ: web_safari, web, web_creator, mweb (EMPTY)
    # BỎ: android_vr (FAIL)
]
...
# Track mtime của cookies.txt để reload nếu file thay đổi.
...
class YouTubeResearcher:
    ...
    def __init__(self, api_key: str, output_dir: str = "./researched_videos",
                 ...
                 subs_populate_concurrency: int = 5):
        ...
        if player_clients:
            self._v15_player_clients = [
                s.strip() for s in player_clients.split(",") if s.strip()
            ]
        else:
            self._v15_player_clients = list(PLAYER_CLIENT_ROTATION_LIST)
        # === v15: API MODE SUBS POPULATE ===
    ...
    def _reload_cookies_if_changed() -> Optional[str]:
        """v15: Check mtime cookies.txt → reload COOKIES_FILE_STR nếu file đổi.
    ...
    def _apply_auth_skip(ydl_opts: dict, player_client: Optional[str] = None) -> dict:
        ...
        Args:
            ...
            player_client: nếu set → dùng 1 client cụ thể
                (vd "tv_embedded", "web_safari").
                Nếu None → dùng ["web"] (giống browser thật nhất,
                bypass bot check tốt nhất khi có cookies).
        """
        ...
        if player_client:
            ...
            # Option A fix: default = ["web"] (giống browser thật).
            # - "web" + cookies = bypass bot check tốt nhất.
            # - "web_safari" hay trigger "Sign in to confirm you're not a bot"
            #   vì YT phân biệt Safari UA với Chrome.
    ...
    def find_existing_audio(audio_root, video, target_filename: str = "",
        ...
        scan_targets = subdirs + [root]  # subdir mới trước, root flat sau
    ...
    def _build_audio_index(audio_root, min_size_bytes: int = 50 * 1024) -> dict:
        ...
        scan_targets = [root] + subdirs  # root flat trước, subdir mới overwrite
        for sub in scan_targets:
            try:
    ...
    def fetch_channel_videos(self, channel_input: str, max_results: int = 20000,
        ...
        populate_concurrency = getattr(self, "_v15_subs_populate_concurrency", 5)
    ...
    def _populate_subs_for_video(self, video_id: str,
                                  player_client: Optional[str] = None) -> dict:
        """v15: Extract subs URLs cho 1 video qua yt-dlp.

        Returns:
            dict {"subtitles": {...}, "automatic_captions": {...}}
            hoặc dict rỗng nếu fail.
        """
        try:
            import yt_dlp
        except ImportError:
        ...
        # Không dùng proxy (để đỡ overhead cho populate phase)
        # Nếu cần proxy: dùng _proxy_for_transcript_fallback()
        ...
        try:
            ...
            err_str = str(e).lower()
            if any(k in err_str for k in ['captcha', 'sign in', 'bot', '429']):
                # captcha → KHÔNG lưu cache, để caller retry
                return {}
            return {}
    ...
    def _populate_subs_parallel(self, videos: list,
        ...
        need_populate = []
        for v in videos:
            if not v.subtitles and not v.automatic_captions:
                need_populate.append(v)
            elif v.subtitles or v.automatic_captions:
                pass  # already has subs (fallback path)

        if not need_populate:
            print(f"  [v15-populate] All {n} videos đã có subs → skip")
            return

        print(f"  [v15-populate] {len(need_populate)}/{n} videos cần populate "
              f"({n - len(need_populate)} đã có sẵn)")
        ...
        def _populate_one(idx: int, v) -> None:
            ...
            t_start_v = time.time()
            try:
                attempt_result = self._populate_subs_for_video(
                    vid, player_client="tv_embedded"
                )
                elapsed_v = time.time() - t_start_v
                subs = attempt_result.get("subtitles") or {}
                auto = attempt_result.get("automatic_captions") or {}
                ...
                if subs or auto:
                    ...
                    print(f"    [v15-populate] ✅ {vid} ({elapsed_v:.1f}s) "
                          f"subs={total_subs} auto={total_auto}{vi_mark}",
                          flush=True)
                else:
                    print(f"    [v15-populate] ⚠️ {vid} ({elapsed_v:.1f}s) "
                          f"EMPTY (no subs, no auto)", flush=True)
            except Exception as e:
                elapsed_v = time.time() - t_start_v
                err_short = str(e)[:100]
                print(f"    [v15-populate] ❌ {vid} ({elapsed_v:.1f}s) "
                      f"tv_embedded error: {err_short}", flush=True)
            try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
            futures = [
                executor.submit(_populate_one, idx, v)
    ...
    def _get_youtube_transcript_via_ytdlp(self, video_id: str,
        ...
        if info is None:
            ...
            # Bound timeout 10s (kể cả khi socket_timeout 60s fail)
            import concurrent.futures
            try:
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(
                    ...
                    try:
                        ...
                        print(f"  [ytdlp-subs] extract_info timeout 10s, killing")
                        if proxy_url:
    ...
    def transcribe_with_youtube(self, video_id: str, audio_path: Path = None,
        ...
        # v15: Retry tối đa 2 attempts yt-dlp với proxy + player_client rotate
        max_attempts = 2
        result = None
        ...
        for a in range(max_attempts):
            if a == 0:
                ...
                # Attempt 2: LUÔN rotate IP + dùng VPN proxy
...
def parse_args():
    ...
    p.add_argument("--subs-populate-concurrency", type=int, default=5,
                   help="v15: Số worker concurrent cho populate subs phase. "
                        "Mặc định: 5. Set thấp nếu YouTube rate-limit.")
    return p.parse_args()
    ...
    for i, (video, target_name, target_filename) in enumerate(bucket_c, 1):
        ...
        try:
            ...
            for dl_attempt in range(1, dl_retries + 1):
                ...
                # v15 FIX: truyền player_client tốt nhất (tv_embedded) để tránh
                # "Requested format is not available" khi default web_safari/web
                # trả EMPTY formats cho 1 số video.
                self._apply_auth_skip(ydl_opts, player_client="tv_embedded")
                self._apply_cookies(ydl_opts)
