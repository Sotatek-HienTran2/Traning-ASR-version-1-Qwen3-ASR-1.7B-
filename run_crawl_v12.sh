...
# =============================================================================
# run_crawl_v12.sh — clone của run_crawl_1.sh + v12 mid-download slow-speed rotation
# -----------------------------------------------------------------------------
# v12 khác v11: bật cơ chế "đổi IP NGAY khi đang tải chậm" thay vì phải đợi
# download xong hoặc fail hẳn.
#
# Chạy giống hệt run_crawl_1.sh về:
#   - Python interpreter, channels file, max_results, max_fetch, video_delay
#   - kill_vpn_fake_ips() ở đầu (kill hết tunnel cũ của user)
#   - Force PATH sang node v24.15.0
#   - KHÔNG truyền --instance-id / --cleanup-on-exit (đơn instance)
#   - Output mặc định của Python: ./youtube_dataset
#
# Các flag v12 truyền thêm (đều có default nên KHÔNG bắt buộc):
#   --audio-slow-speed-kbps N        # rolling avg < N KB/s → rotate
#                                    # default: 50.0, 0 = tắt
#   --audio-slow-window-seconds N    # cửa sổ tính rolling avg
#                                    # default: 30.0
#   --audio-max-rotate-per-video N   # số lần rotate tối đa do slow-speed/video
#                                    # default: 3, 0 = không giới hạn
#
# Override qua env, ví dụ:
#   AUDIO_SLOW_SPEED_KBPS=30 AUDIO_SLOW_WINDOW_SECONDS=45 \
#   AUDIO_MAX_ROTATE_PER_VIDEO=5 ./run_crawl_v12.sh
# =============================================================================

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PY=/home/hientran/miniconda3/envs/crawl/bin/python3
CHANNELS_FILE=/home/hientran/sythetic_crawl_data/channels_audio/channels_AI_7.txt
MAX_RESULTS=5000
MAX_FETCH=3000
VIDEO_DELAY=8
...
AUDIO_SLOW_SPEED_KBPS="${AUDIO_SLOW_SPEED_KBPS:-50.0}"
...
echo "[run_crawl] starting crawler (v12: mid-download slow-speed rotation)..."
