                ...
                if self._inner._has_tun0():
                    ...
                    if ip and ip != real_ip:
                        ...
                        try:
                            ...
                            print(f"    [vpn-connect] openvpn PID {new_pid} died sau 1s "
...
class YouTubeResearcher:
    ...
    def __init__(self, api_key: str, output_dir: str = "./researched_videos",
        ...
        # Pre-check fail streak (reset mỗi video, skip pre-check sau N fail liên tiếp)
        self._precheck_fail_streak = 0
        ...
        while fetched < max_results:
            ...
            try:
                ...
                if any(kw in err for kw in transient_kws):
                    ...
                    # v17-fix: Phase 1 dùng YouTube Data API (googleapiclient),
                    # phải rotate metadata rotator (_rotator), không phải
                    # transcript rotator (_transcript_rotator).
                    try:
                        if (self._rotator is not None
                                and hasattr(self._rotator,
                                            "force_rotate")):
                            self._rotator.force_rotate(
    ...
    @staticmethod
    def _fetch_transcript_innertube(video_id: str, lang: str = "vi",
                                    session=None) -> Optional[list]:
        """Lấy transcript qua Innertube get_transcript API (POST).

        Dùng thay thế /api/timedtext khi bị 429.
        Trả về list[dict(start, dur, text)] hoặc None nếu fail.
        """
        import base64, struct, json as _json
        import requests as _req

        # Encode protobuf params: field1=video_id (string), field2=lang (string)
        def _encode_string(field_num: int, s: str) -> bytes:
            b = s.encode("utf-8")
            tag = (field_num << 3) | 2  # wire type 2 = length-delimited
            return bytes([tag, len(b)]) + b

        # params protobuf: {1: video_id, 2: lang}
        proto = _encode_string(1, video_id) + _encode_string(2, lang)
        params = base64.b64encode(proto).decode()
        ...
        headers = {
            ...
            "User-Agent": "Mozilla/5.0 (X11; Linux x86_64) Chrome/125.0",
        ...
        try:
            ...
            resp = sess.post(url, json=payload, headers=headers, timeout=(10, 20))
            if resp.status_code != 200:
                return None
            data = resp.json()

            # Parse response: actions[0].updateEngagementPanelAction.content
            ...
            try:
                ...
                for action in actions:
                    panel = action.get("updateEngagementPanelAction", {})
                    content = panel.get("content", {})
                    tr = content.get("transcriptRenderer", {})
                    tc = tr.get("content", {})
                    sp = tc.get("transcriptSearchPanelRenderer", {})
                    body = sp.get("body", {})
                    seg_list = body.get("transcriptSegmentListRenderer", {})
                    init_segs = seg_list.get("initialSegments", [])
                    for s in init_segs:
                        seg = s.get("transcriptSegmentRenderer", {})
                        if not seg:
                            continue
    ...
    def _get_youtube_transcript_via_ytdlp(self, video_id: str,
        ...
        if info is None:
            ...
            # === v17: request spacing (anti rate-limit) ===
            ydl_opts['sleep_requests'] = int(os.environ.get('YTDLP_SLEEP_REQUESTS', '2'))
            ydl_opts['sleep_interval'] = int(os.environ.get('YTDLP_SLEEP_INTERVAL', '3'))
            ydl_opts['max_sleep_interval'] = int(os.environ.get('YTDLP_MAX_SLEEP_INTERVAL', '10'))
            ydl_opts['extractor_retries'] = 3
            # v15: dùng source_address policy routing thay vì proxy
            # force_direct=True → bỏ qua VPN, dùng real IP
            _tr_src = None
            if not force_direct and self._transcript_rotator and hasattr(self._transcript_rotator, 'get_source_address'):
                _tr_src = self._transcript_rotator.get_source_address()
            if _tr_src:
                ydl_opts["source_address"] = _tr_src
                ydl_opts["force_ipv4"] = True
                import socket as _sv
                _orig_getaddrinfo = _sv.getaddrinfo
                def _ipv4_only_getaddrinfo(host, port, family=0, type=0, proto=0, flags=0):
                    return _orig_getaddrinfo(host, port, _sv.AF_INET, type, proto, flags)
            elif proxy_url:
                ydl_opts["proxy"] = proxy_url

            import concurrent.futures
            _extract_timeout = int(os.environ.get("YTDLP_EXTRACT_TIMEOUT", "10"))
            _precheck_timeout = float(os.environ.get("YTDLP_PRECHECK_TIMEOUT", "3"))
            _precheck_max_fails = int(os.environ.get("YTDLP_PRECHECK_MAX_FAILS", "1"))
            _precheck_skipped = False

            if _tr_src:
                _sv.getaddrinfo = _ipv4_only_getaddrinfo
            try:
                # Pre-check TCP connect tới YouTube (chỉ khi dùng source_address)
                if _tr_src:
                    _streak = getattr(self, '_precheck_fail_streak', 0)
                    if _precheck_max_fails > 0 and _streak >= _precheck_max_fails:
                        print(f"    [pre-check] streak={_streak}/{_precheck_max_fails} → DC vẫn bị chặn, return vpn_timeout ngay")
                        return None, "vpn_timeout"
                    else:
                        _pre_ok = False
                        try:
                            _ps = _sv.socket(_sv.AF_INET, _sv.SOCK_STREAM)
                            _ps.settimeout(_precheck_timeout)
                            try:
                                _ps.bind((_tr_src, 0))
                                _ps.connect(("youtube.com", 443))
                                _pre_ok = True
                            finally:
                                _ps.close()
                        except OSError:
                            pass
                        if not _pre_ok:
                            self._precheck_fail_streak = _streak + 1
                            print(f"    [pre-check] FAIL: timed out (streak={self._precheck_fail_streak}/{_precheck_max_fails})")
                            return None, "vpn_timeout"
                        else:
                            self._precheck_fail_streak = 0
                            print(f"    [pre-check] OK (streak reset)")

                # Re-verify source_address vẫn bindable trước khi chạy yt-dlp
                if _tr_src:
                    _src_still_ok = False
                    try:
                        _ts = _sv.socket(_sv.AF_INET, _sv.SOCK_STREAM)
                        try:
                            _ts.bind((_tr_src, 0))
                            _src_still_ok = True
                        finally:
                        ...
                        pass
                    if not _src_still_ok:
                        ydl_opts.pop("source_address", None)
                        ydl_opts.pop("force_ipv4", None)
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                    ...
                    try:
                        ...
                        if proxy_url:
                            self._mark_transcript_proxy_dead(proxy_url)
                        if _tr_src and not _precheck_skipped:
                            return None, "extract_failed"
                        return None, "vpn_timeout"
            except Exception as e:
                err_str = str(e)
                ...
                if _tr_src:
                    _sv.getaddrinfo = _orig_getaddrinfo
        ...
        for key_idx, (key, _dict, source_type, score, lang_code) in enumerate(scored):
            ...
            print(f"  [ytdlp-subs] ({key_idx + 1}/{len(scored)}) "
                  ...
                  f"url_preview={sub_url[:80] if sub_url else 'None'} "
            ...
            content = None
            inline_rotates = 0
            max_inline_rotates = int(os.environ.get("TRANSCRIPT_MAX_ATTEMPTS", "5"))
            current_proxy = proxy_url
            # pre-compute source address cho cả download loop lẫn innertube fallback
            # v17-fix: khi force_direct=True, bỏ qua VPN cho cả download sub
            # (không chỉ extract_info) vì VPN DC có thể bị block ở tầng TCP,
            # sub download cũng sẽ timeout ConnectionError → phí thời gian.
            _sub_tr_src = None
            if (not force_direct
                    and self._transcript_rotator
                    and hasattr(self._transcript_rotator, 'get_source_address')):
                _sub_tr_src = self._transcript_rotator.get_source_address()
            ...
            try:
                ...
                # Chọn session: fake IP nếu có tunnel, fallback real IP
                # v17-fix: khi force_direct=True, _sub_tr_src=None nên
                # _bound_sess=None → dùng real IP luôn (tránh timeout VPN).
                _bound_sess = _make_bound_session(_sub_tr_src) if _sub_tr_src else None
                _dl_sess = _bound_sess or _requests

                while inline_rotates <= max_inline_rotates:
                    try:
                        # Lần đầu thử với cookies, nếu 429 → thử không cookies.
                        _cookies = cookies_dict if inline_rotates == 0 else {}
                        _ip_label = f"fake({_sub_tr_src})" if _sub_tr_src else "real"
                        resp = _dl_sess.get(sub_url, cookies=_cookies,
                                            timeout=(10, 20))
                        print(f"    [ytdlp-subs] download via {_ip_label} → status={resp.status_code}")
                        if resp.status_code == 429:
                            if inline_rotates == 0 and _sub_tr_src:
                                # Fake IP bị 429: thử real IP không cookies
                                inline_rotates += 1
                                _dl_sess = _requests
                                _sub_tr_src = None
                                _ip_label = "real(fallback)"
                                print(f"    [ytdlp-subs] HTTP 429 via fake IP (key='{key}') → fallback real IP không cookies")
                                continue
                            elif inline_rotates == 0:
                                # Real IP bị 429: retry không cookies
                                inline_rotates += 1
                                print(f"    [ytdlp-subs] HTTP 429 (key='{key}') → retry không cookies")
                                continue
                            else:
                                print(f"    [ytdlp-subs] HTTP 429 (key='{key}') — hết retry → skip key")
                                break
            ...
            # v17: Innertube fallback khi timedtext bị 429/block
            if content is None and lang_code:
                print(f"    [ytdlp-subs] timedtext failed → thử Innertube get_transcript (lang={lang_code})")
                _it_segs = self._fetch_transcript_innertube(
                    video_id, lang=lang_code,
                    session=_make_bound_session(_sub_tr_src) if _sub_tr_src else None,
                )
                if _it_segs:
                    print(f"    [ytdlp-subs] Innertube OK: {len(_it_segs)} segments")
                    best_result = {"segments": _it_segs, "is_auto": is_auto,
                                   "lang": lang_code, "key": key, "source": "innertube"}
                    break
                else:
                    print(f"    [ytdlp-subs] Innertube also failed → skip key")
                    continue
        ...
        if not best_result:
            # v17-fix: Phân biệt "video KHÔNG có sub" vs "có sub nhưng IP bị chặn download".
            # - tried_keys rỗng → extract_info tìm thấy VI keys → nhưng download timedtext
            #   fail hết (ConnectionError/timeout) → LÀ IP BLOCK, không phải "no subs".
            #   → trả "extract_failed" để caller force_rotate IP.
            # - tried_keys rỗng → thực sự không có VI sub nào → "no_subs".
            if tried_keys:
                print(f"  [ytdlp-subs] ❌ tried {len(tried_keys)} VI key(s) "
                      f"({tried_keys}) — all failed (download/parse error "
                      f"sau {max_inline_rotates} lần IP rotate) → IP có thể bị block "
                      f"ở timedtext API, trả extract_failed để rotate IP")
                return None, "extract_failed"
            else:
                print(f"  [ytdlp-subs] ❌ tried {len(tried_keys)} VI key(s) "
                      f"({tried_keys}) — all failed (download/parse error "
    ...
    def transcribe_with_youtube(self, video_id: str, audio_path: Path = None,
        ...
        """
        v14: Lấy phụ đề sẵn có của YouTube với multi-attempt retry + API fallback.

        Priority:
          1. yt-dlp subtitles download (retry 2 lần với proxy rotation).
          2. youtube-transcript-api fallback (nếu --no-api-fallback=False).
             Engine khác (timedtext API), thường bypass captcha tốt hơn yt-dlp
             khi IP/proxy đã bị Google flag.
        ...
        # Reset pre-check streak mỗi video mới
        self._precheck_fail_streak = 0

        print(f"  Fetching YouTube transcript via yt-dlp (langs={lang})...")

        # === v17: Every-N-audio IP rotate (chu kỳ) ===
        # Cứ mỗi N audio đã xử lý → tự động force_rotate IP fake của transcript_rotator.
        # v17: Default 20 (tăng từ 10 vì không rotate mỗi attempt nữa).
        rotate_every_n = int(os.environ.get("TRANSCRIPT_ROTATE_EVERY", "20"))
        # Thread-safe counter increment
        if not hasattr(self, '_v17_counter_lock'):
            self._v17_counter_lock = threading.Lock()
        with self._v17_counter_lock:
        ...
        # v17-fix: Các status này cho thấy IP/DC bị block → rotate tunnel
        # để lấy IP mới. Client_empty KHÔNG rotate (do client không hỗ trợ).
        blocked_statuses = {"extract_failed", "vpn_timeout"}
        ...
        for a in range(max_attempts):
            ...
            # v17: CHỈ rotate IP khi có dấu hiệu bị block (status trước là extract_failed)
            # KHÔNG rotate khi client_empty (đó là do client không hỗ trợ, không phải IP bị block)
            # KHÔNG rotate khi parallel mode (sẽ phá route workers khác)
            _use_direct = False  # v17-fix: fallback real IP khi force_rotate fail
            if a > 0 and last_status in blocked_statuses and not _parallel_mode:
                if self._transcript_rotator and hasattr(self._transcript_rotator, 'force_rotate'):
                    _rotate_ok = False
                    try:
                        print(f"    [v17-rotate] IP bị block (status={last_status}) "
                              f"→ force_rotate trước attempt {a+1}")
                        self._transcript_rotator.force_rotate(
                            f"transcript-blocked-attempt-{a+1}")
                        _rotate_ok = True
                    except Exception as _re:
                        print(f"    [v17-rotate] force_rotate FAILED: {_re}")
                        print(f"    [v17-rotate] → fallback real IP cho attempt này")
                        # v17-fix: VPN rotate fail → thử real IP thay vì dùng
                        # connection cũ đã block (sẽ timeout vô ích).
                        _use_direct = True
                # Backoff sau rotate (chỉ sleep nếu rotate OK hoặc không có rotator)
                # Khi rotate fail, skip sleep để thử real IP ngay.
                if not _use_direct:
                    backoff = backoff_seconds
                    if backoff > 0:
                        print(f"    [v17-retry] sleeping {backoff}s before attempt {a+1}")
                        time.sleep(backoff)
            elif a > 0:
                time.sleep(0.1)

            proxy_str = "REAL-IP" if _use_direct else "VPN-FAKE"
            ...
            result, status = self._get_youtube_transcript_via_ytdlp(
                ...
                player_client=pc,
                force_direct=_use_direct,  # v17-fix: dùng real IP khi VPN rotate fail
            )
            all_statuses.append(status)
            last_status = status
            if status == "ok":
                break
            if status == "vpn_timeout":
                print(f"  [v17-transcript] status='vpn_timeout' → DC bị chặn, thử real IP")
                result, status = self._get_youtube_transcript_via_ytdlp(
                    video_id, proxy_url=None, info_cached=cached,
                    player_client=pc, force_direct=True,
                )
                all_statuses.append(status)
                last_status = status
                if result is not None:
                    break
                # v17-fix: real IP cũng fail → rơi xuống attempt tiếp theo
                # để force_rotate sang VPN server khác (thay vì break vô điều kiện)
                print(f"  [v17-transcript] real IP cũng fail "
                      f"(status={status}) → rơi xuống attempt tiếp theo để rotate VPN")
                continue
            if status == "client_empty":
                print(f"  [v17-transcript] status='client_empty' "
                      f"(client={pc_str} trả EMPTY) → thử client khác (KHÔNG đổi IP)")
                continue
            if status == "no_subs":
                print(f"  [v17-transcript] status='no_subs' (video thật sự không có sub). "
...
def _process_videos_pipeline(self, output_dir, run_timestamp="",
    ...
    if _bucket_b_todo:
        ...
        def _process_bucket_b_item(item):
            ...
            if result:
                ...
                print(f"  [B-{idx}] No YouTube transcript available")
    ...
    for i, (video, target_name, target_filename) in enumerate(bucket_c, 1):
        ...
        # v17: thêm random jitter ±50% để instance chạy song song không
        # bị fingerprint (pattern delay cố định dễ bị phát hiện).
        if i > 1 and video_delay > 0:
            import random as _random_mod
            jitter = _random_mod.uniform(0.5, 1.5)
            delay = video_delay * jitter
            time.sleep(delay)
        ...
        try:
            ...
            for dl_attempt in range(1, dl_retries + 1):
                ...
                ydl_opts = {
                    ...
                    # Google phát hiện IP download quá nhanh → rate-limit.
                    # Các tham số này cách đều HTTP requests + cap tốc độ.
                    # Override qua env: YTDLP_SLEEP_REQUESTS, YTDLP_SLEEP_INTERVAL, ...
                    'sleep_requests': int(os.environ.get('YTDLP_SLEEP_REQUESTS', '1')),
                    'sleep_interval': int(os.environ.get('YTDLP_SLEEP_INTERVAL', '3')),
                    'max_sleep_interval': int(os.environ.get('YTDLP_MAX_SLEEP_INTERVAL', '8')),
                    'sleep_interval_subtitles': int(os.environ.get('YTDLP_SLEEP_SUBS', '1')),
                    # Cap download speed ~5MB/s để tránh burst quá aggressive
                    'throttledratelimit': int(os.environ.get('YTDLP_THROTTLED_RATE', str(5 * 1024 * 1024))),
                }
