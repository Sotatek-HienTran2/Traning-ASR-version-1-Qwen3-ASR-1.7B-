...
class VPNRotator:
    ...
    def __init__(
        ...
        # === v4: Per-server cooldown & blacklist tracking ===
        self._server_cooldown: Dict[int, float] = {}
        self._cooldown_seconds: int = 300
        self._consecutive_fails: Dict[int, int] = {
            i: 0 for i in range(len(self._ovpn_files))
        }
        self._blacklist: set = set()
        self._cooldown_threshold: int = 3
        self._captcha_keywords = [
            'sign in to confirm', 'not a bot', 'captcha', 'challenge',
            'consent', 'agree to', 'verify you', 'unusual traffic',
        ]
        self._captcha_hit_count: int = 0
    ...
    def mark_success(self, proxy_url: Optional[str]):
        """v4: Báo proxy OK -> reset consecutive_fails cho current_idx."""
        with self._lock:
            if self._current_idx is not None:
                self._consecutive_fails[self._current_idx] = 0

    def remove_proxy(self, proxy_url: Optional[str]):
        """v4: BLACKLIST server hien tai vinh vien trong session.

        Khong con la no-op nhu v3. Caller (YouTubeResearcher) se goi khi gap
        captcha hoac loi proxy chet that.
        """
        _idtag = (
            f"[{self._instance_id}]" if getattr(self, "_instance_id", None)
            else "[VPN]"
        ...
        with self._rotate_lock:
            with self._lock:
                if self._current_idx is not None:
                    self._blacklist.add(self._current_idx)
                    log.warning(
                        "VPN %s: remove_proxy(%s) -> BLACKLIST server %s (%s)",
                        _idtag, proxy_url, self._current_idx,
                        self._ovpn_files[self._current_idx].name,
                    )
                    self._rotate_locked()

    def is_captcha_error(self, err) -> bool:
        """v4: Check error co phai captcha/bot check khong."""
        err_str = str(err).lower()
        return any(kw in err_str for kw in self._captcha_keywords)

    def force_rotate(self, reason: str = "captcha"):
        """v4: Rotate NGAY khong cho worker idle (cho captcha/bot check).

        - Tang _consecutive_fails[current_idx] +1
        - Neu >= cooldown_threshold -> cooldown server do N giay
        - Rotate sang server khac (uu tien server it fail)
        """
        _idtag = (
            f"[{self._instance_id}]" if getattr(self, "_instance_id", None)
            else "[VPN]"
        ...
        with self._rotate_lock:
            with self._lock:
                if self._current_idx is not None:
                    ...
                    self._consecutive_fails[cur] = self._consecutive_fails.get(cur, 0) + 1
                    if self._consecutive_fails[cur] >= self._cooldown_threshold:
                        self._server_cooldown[cur] = time.time() + self._cooldown_seconds
                        log.warning(
                            "VPN %s: server %s fail %d lan -> COOLDOWN %ds",
                            _idtag, cur,
    ...
    def increment_captcha_hit(self):
        with self._lock:
            self._captcha_hit_count += 1

    def get_captcha_hit_count(self) -> int:
        with self._lock:
            return self._captcha_hit_count

    def stats(self) -> List[Dict]:
        """v4: them consecutive_fails + cooldown_remaining_s + blacklisted."""
        now = time.time()
        result = []
        for i, ovpn in enumerate(self._ovpn_files):
            cooldown_until = self._server_cooldown.get(i, 0)
            cooldown_remaining = max(0, int(cooldown_until - now))
            result.append({
                "url": f"vpn://{ovpn.name}",
                "uses": self._usage_count.get(i, 0),
                "fails": self._fail_count.get(i, 0),
                "consecutive_fails": self._consecutive_fails.get(i, 0),
                "cooldown_remaining_s": cooldown_remaining,
                "blacklisted": i in self._blacklist,
    ...
    def _prepare_config(self, ovpn_path: Path) -> Path:
        """
        Tạo temp .ovpn file KHÔNG có `up`/`down` DNS update script.

        Lý do: File .ovpn gốc của ProtonVPN có dòng:
            up /etc/openvpn/update-resolv-conf
            down /etc/openvpn/update-resolv-conf
        Script này cần root (gọi /sbin/resolvconf) → user thường bị
        "Interactive authentication required" → openvpn exit.

        Returns:
            Path tới temp .ovpn (đã strip DNS script, keep nguyên các thứ khác)
        """
        # Đọc file gốc
        original = ovpn_path.read_text(encoding="utf-8", errors="replace")

        # Comment out các dòng up/down DNS script (nếu có)
        cleaned_lines = []
        for line in original.splitlines():
            ...
            if (
                ...
                # Comment out
                cleaned_lines.append(f"# {line}  # disabled by vpn_rotator (no root for resolvconf)")
            else:
                cleaned_lines.append(line)
        cleaned = "\n".join(cleaned_lines) + "\n"

        # Ghi ra temp file cùng thư mục để không lỗi path của cert
        temp_path = ovpn_path.parent / f".{ovpn_path.stem}.no-dns.ovpn"
        temp_path.write_text(cleaned, encoding="utf-8")
        return temp_path

    def _has_tun0(self) -> bool:
        """Check xem interface tun0 đã lên chưa (dấu hiệu tunnel đã establish)."""
        try:
            result = subprocess.run(
                ["ip", "link", "show", "tun0"],
    ...
    def _connect_server(self, idx: int, retry: int = 0) -> bool:
        ...
        # Đợi tunnel lên — check cả tun0 interface VÀ IP change
        # (vì IP change là dấu hiệu traffic đã route qua tunnel)
        for i in range(CONNECT_TIMEOUT):
            time.sleep(1)
            # Ưu tiên check tun0 (chính xác hơn)
            if self._has_tun0():
                # Verify IP đã đổi
                ...
                if ip and ip != real_ip:
                    ...
                    log.info(
                        "VPN %s: ✓ connected to %s, IP=%s, PID=%s (sau %ds, attempt %d)",
                        _idtag, ovpn.name, ip, new_pid, i + 1, retry + 1,
                    )
                    return True
                # tun0 lên nhưng IP chưa đổi → route có thể chưa add xong
                # tiếp tục đợi
            # Fallback: check IP change (cho trường hợp tun0 tên khác)
            elif i >= 3:  # Đợi ít nhất 3s để tun0 lên
                ip = self._get_current_ip()
                real_ip = self._last_known_real_ip
                if ip and real_ip and ip != real_ip:
                    ...
                    self._last_connect_time = time.time()
                    log.info(
                        "VPN %s: ✓ connected (no tun0 check) to %s, IP=%s, PID=%s "
                        "(sau %ds, attempt %d)",
                        _idtag, ovpn.name, ip, new_pid, i + 1, retry + 1,
                    )
                    return True
    ...
    def _rotate_locked(self):
        ...
        v4 IMPROVEMENTS:
          - Skip server trong _blacklist (vinh vien trong session)
          - Skip server dang cooldown (consecutive_fails >= threshold)
        ...
        # v4: Tinh available servers (loai blacklist + cooldown)
        now = time.time()
        available = []
        for i in range(len(self._ovpn_files)):
            if i == old_idx:
                continue
            if i in self._blacklist:
                continue
            if i in self._server_cooldown and now < self._server_cooldown[i]:
                continue
            available.append(i)

        # Neu TAT CA deu bi skip -> reset blacklist+cooldown (cho cycle moi)
        if not available:
            log.warning(
                "VPN %s: ALL servers unavailable -> RESET blacklist+cooldown",
                _idtag,
            )
            self._blacklist.clear()
            self._server_cooldown.clear()
            available = [i for i in range(len(self._ovpn_files)) if i != old_idx]
            if not available:
                available = list(range(len(self._ovpn_files)))

        # Reset consecutive_fails cua old_idx
        if old_idx is not None:
            self._consecutive_fails[old_idx] = 0

        # Chon server theo strategy
        if self.strategy == "least_used":
            available.sort(key=lambda i: (self._consecutive_fails.get(i, 0),
                                          self._usage_count.get(i, 0)))
            new_idx = available[0]
        elif self.strategy == "sequential":
            available.sort()
            new_idx = available[0]
        else:  # random: uu tien server consecutive_fails = 0
            low_fails = [i for i in available
                         if self._consecutive_fails.get(i, 0) == 0]
            if low_fails:
                new_idx = random.choice(low_fails)
            else:
