#!/usr/bin/env python3
"""
YouTube Researcher -- AUDIO + YOUTUBE SUBS (MULTI-ROTATOR) -- v5 AUDIO-IP-CONTROLLER
====================================================================================

v5 = youtube_researcher_audio_subs_multi_rotator_v4.py + AudioIPController.

Vấn đề của v3/v4: Khi cần "đổi IP" cho audio download, code chỉ rotate sang
**server VPN khác trong cùng proton_config** (vd: từ `us-free-5` sang `nl-free-7`).
Đây thực chất là **đổi VPN exit node**, chứ KHÔNG phải đổi về **IP thật** (default
route khi không có VPN tunnel). Vì vậy user vẫn bị "Sign in to confirm" v.v.
...
So với v4, bổ sung:
  ...
  - 5 tham số CLI mới (--audio-min-speed-mbps, --audio-fake-before-real,
...
class AudioIPController:
    ...
    def __init__(
        ...
        # === v5.1: Rolling speed window ===
        # Lưu các sample (t_elapsed, bytes_dl) cho mỗi chunk progress.
        # Khi tính tốc độ, lấy sample cũ nhất trong window → sample mới nhất
        # rồi chia cho (t_new - t_old). Làm mượt dao động tức thời.
        self._speed_samples: list = []  # [(t_elapsed_sec, bytes_dl), ...]
    ...
    def on_download_start(self) -> "str | None":
        ...
        if self._state == self.STATE_REAL:
            ...
            print(f"    [audio-ip] STATE=REAL (IP thật, no VPN tunnel)")
            return None
        else:
            # FAKE: cần VPN tunnel active
            # Gọi next() 1 lần duy nhất: trigger connect nếu cần, hoặc
            # tăng counter request. VPNRotator.next() trả None (vì OpenVPN
            # là system tunnel) — caller vẫn cần acquire guard để giữ tunnel.
            next_result = None
            ...
            try:
                ...
                print(f"    [audio-ip] STATE=FAKE (server={server_name}, ip={cur_ip})")
            except Exception:
                print(f"    [audio-ip] STATE=FAKE")
            return next_result
    ...
    def on_download_complete(self, bytes_dl: int, elapsed_s: float, ok: bool):
        ...
        # Nếu chunk-progress hook đã flag "slow" thì coi như speed không OK
        if self._slow_flag and ok:
            speed_ok = False

        if self._state == self.STATE_REAL:
            if speed_ok:
                # Real IP OK, giữ nguyên
                print(f"    [audio-ip] REAL OK: {bytes_dl//1024}KB in {elapsed_s:.1f}s "
                      f"= {speed_mbps:.2f} MB/s (>= {self.min_speed_mbps} MB/s) — GIỮ")
                self._slow_flag = False
                return
            # Real IP chậm hoặc fail → chuyển sang FAKE
            print(f"    [audio-ip] REAL FAIL/SLOW: {bytes_dl//1024}KB in {elapsed_s:.1f}s "
                  f"= {speed_mbps:.2f} MB/s (ok={ok}, slow_flag={self._slow_flag}) "
                  f"—> chuyển sang FAKE")
            self._state = self.STATE_FAKE
            self._consecutive_fake_slow = 0
            self._slow_flag = False
            # disconnect() để chắc chắn VPN tắt trước khi next() reconnect
            self._ensure_vpn_disconnected()
            return

        # State == FAKE
        if speed_ok:
            # Fake IP OK, giữ nguyên VPN tunnel
            print(f"    [audio-ip] FAKE OK: {bytes_dl//1024}KB in {elapsed_s:.1f}s "
                  f"= {speed_mbps:.2f} MB/s — GIỮ VPN tunnel")
            self._consecutive_fake_slow = 0
            self._slow_flag = False
            return

        # Fake IP chậm hoặc fail
        self._consecutive_fake_slow += 1
        if self._consecutive_fake_slow >= self.fake_before_real:
            # Đã đủ N lần fake chậm → cycle về REAL
            print(f"    [audio-ip] FAKE SLOW {self._consecutive_fake_slow}/{self.fake_before_real} "
                  f"lần liên tiếp: {bytes_dl//1024}KB in {elapsed_s:.1f}s "
                  f"= {speed_mbps:.2f} MB/s —> CYCLE về REAL (cycle {self.fake_before_real + 1})")
            self._state = self.STATE_REAL
            self._consecutive_fake_slow = 0
            self._slow_flag = False
        ...
        # Chưa đến ngưỡng cycle → force_rotate sang VPN server khác
        print(f"    [audio-ip] FAKE SLOW {self._consecutive_fake_slow}/{self.fake_before_real}: "
              f"{bytes_dl//1024}KB in {elapsed_s:.1f}s = {speed_mbps:.2f} MB/s "
              f"—> force_rotate sang VPN server khác")
        self._slow_flag = False
        self._total_rotates += 1
        try:
            ...
            print(f"    [audio-ip] force_rotate error: {e}")

    def on_chunk_progress(self, bytes_dl: int, elapsed_s: float, speed_bps: float):
        """Callback mid-download từ yt-dlp progress hook.

        Tính tốc độ TRUNG BÌNH (rolling average) trong cửa sổ
        `speed_avg_window_seconds` (mặc định 10s) thay vì dùng
        tốc độ tức thời (dao động mạnh do buffering/IO).

        Đánh dấu _slow_flag=True nếu:
          - Đã tải >= min_bytes_for_speed
          - Đã chạy >= min_window_seconds
          - Tốc độ TB trong window < min_speed_mbps

        Lưu ý: rolling avg = (bytes_now - bytes_oldest_in_window) /
        (t_now - t_oldest_in_window). Nếu window chỉ có 1 sample
        (download mới bắt đầu) → fallback dùng speed_bps tức thời.
        """
        # === v5.1: Lưu sample vào rolling window ===
        # Trim các sample quá cũ (ngoài window) trước khi tính.
        # Window = [elapsed_s - speed_avg_window_seconds, elapsed_s]
        # Sample t < cutoff = (elapsed_s - window) thì BỎ.
        cutoff = elapsed_s - self.speed_avg_window_seconds
        if self._speed_samples and self._speed_samples[0][0] < cutoff:
            # Bỏ TẤT CẢ sample có t < cutoff (KHÔNG giữ anchor)
            # Lý do: anchor cũng đã cũ hơn window → không còn ý nghĩa
            # cho rolling average. Khi window trượt qua anchor, speed
            # tự nhiên thay đổi theo data mới.
            self._speed_samples = [
                (t, b) for (t, b) in self._speed_samples if t >= cutoff
            ]
        # Thêm sample hiện tại
        self._speed_samples.append((elapsed_s, bytes_dl))

        if bytes_dl < self.min_bytes_for_speed:
            return
        if elapsed_s < self.min_window_seconds:
            return

        # === Tính rolling average ===
        avg_speed_mbps = 0.0
        if len(self._speed_samples) >= 2:
            t_old, b_old = self._speed_samples[0]
            t_new, b_new = self._speed_samples[-1]
            dt = t_new - t_old
            db = b_new - b_old
            if dt > 0.1 and db > 0:
                avg_speed_mbps = (db / dt) / (1024 * 1024)
            # Nếu db <= 0 (không có byte mới trong window) → speed = 0
        else:
            # Chỉ có 1 sample → fallback speed_bps tức thời
            if speed_bps and speed_bps > 0:
                avg_speed_mbps = speed_bps / (1024 * 1024)
            elif elapsed_s > 0:
                avg_speed_mbps = (bytes_dl / elapsed_s) / (1024 * 1024)

        if avg_speed_mbps < self.min_speed_mbps:
            self._slow_flag = True
            # Log CHỈ 1 LẦN đầu để tránh spam
            if not getattr(self, "_slow_logged_this_dl", False):
                window_used = (
                    self._speed_samples[-1][0] - self._speed_samples[0][0]
                    if len(self._speed_samples) >= 2 else 0
                )
                print(f"    [audio-ip] CHUNK-SLOW (avg over {window_used:.1f}s): "
                      f"{bytes_dl//1024}KB in {elapsed_s:.1f}s = "
                      f"{avg_speed_mbps:.2f} MB/s (< {self.min_speed_mbps} MB/s, "
                      f"window={self.speed_avg_window_seconds}s)")
                self._slow_logged_this_dl = True

    def on_download_start_reset_slow_log(self):
        """Reset slow_logged flag + clear speed samples khi bắt đầu
        download mới (gọi từ progress hook)."""
...
def parse_args():
    ...
    p.add_argument("--vpn-rotate-every", type=int, default=40,
    ...
    p.add_argument("--vpn-strategy", choices=["random", "sequential", "least_used"],
                   default="random")
    p.add_argument("--video-delay", type=int, default=7,
    ...
    p.add_argument("--audio-min-speed-mbps", type=float, default=1.5,
    ...
    p.add_argument("--audio-min-window-seconds", type=float, default=5.0,
                   help="v5: Tối thiểu thời gian (giây) trước khi đánh giá tốc độ. "
                        "Mặc định: 5.0")
    p.add_argument("--audio-speed-avg-window-seconds", type=float, default=10.0,
...
def _process_videos_pipeline(self, output_dir, run_timestamp="",
    ...
    for i, video in enumerate(self._filtered_videos, 1):
        ...
        try:
            import yt_dlp
            # v4: tang tu 3 -> 5 retries cho audio download
            dl_retries = 5
            info = None
            info_cache = {}
                ...
                try:
                    ...
                    if audio_path and audio_path.exists():
                        try:
                            ...
                            pass
                    if bytes_dl <= 0:
                        bytes_dl = _dl_state["bytes_dl_max"]
