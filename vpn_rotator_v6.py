...
class VPNRotator:
    ...
    def _disconnect(self):
        ...
        v6 FIX: Khi không có PID (lần đầu connect, tunnel chưa tồn tại),
        TUYỆT ĐỐI KHÔNG pkill tất cả openvpn của user. Chỉ kill PID cụ
        thể đã được track. Nếu pid=None → return ngay, không làm gì.

        Lý do: IsolatedVPNRotator bao bọc method này. Mỗi instance có
        tunnel riêng. pkill tất cả sẽ giết tunnel của instance khác →
        vòng lặp kill lẫn nhau vô tận.

        Chú ý quan trọng:
        - Phải track PID của từng openvpn instance qua self._current_pid,
          chỉ kill đúng PID đó.
        - Sau khi kill, phải `wait` cho process chết hẳn (tối đa
          KILL_TIMEOUT giây) trước khi start cái mới.
        """
        ...
        if pid is None:
            # v6 FIX: Không có PID → không kill gì cả.
            # Trước đây pkill -9 tất cả openvpn của user → giết nhầm
            # tunnel instance khác → vòng lặp kill lẫn nhau.
            # Giờ chỉ log và return. IsolatedVPNRotator đã cleanup policy
            # routes riêng, không cần kill gì thêm.
            return
        ...
        try:
            ...
            try:
                ...
                # v6 FIX: Process thuộc user khác → KHÔNG fallback pkill.
                # Chỉ log warning, không kill gì thêm.
                log.warning("VPN: PID %s thuộc user khác, không thể kill", pid)
                return
            ...
            log.warning("Lỗi khi kill PID %s: %s (v6: KHÔNG fallback pkill)", pid, e)
            # v6 FIX: KHÔNG gọi _disconnect_fallback_pkill()
            # Chỉ clear PID, để lần connect sau tự cleanup

    def _disconnect_fallback_pkill(self):
        """v6: NO-OP — không bao giờ pkill tất cả openvpn.

        Trước đây method này pkill -9 tất cả openvpn process của user
        hiện tại → giết nhầm tunnel của instance khác. v6 giữ method
        này như no-op để tương thích ngược, không gọi đến nữa.
        """
        pass
