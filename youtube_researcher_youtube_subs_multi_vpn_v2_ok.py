...
- **Dùng bản dịch / phụ đề SẴN CÓ của YouTube (tiếng Việt) thay cho Soniox API**
    + Lấy qua yt-dlp subtitles download (cookies + ProtonVPN tunnel, retry 2 lần)
    + KHÔNG dùng youtube-transcript-api
    + Không tải audio về -> không tốn dung lượng disk
    + Không cần SONIOX_API_KEY
- **LUÔN dùng ProtonVPN OpenVPN tunnel** (./proton_config/*.ovpn) để fake IP.
  KHÔNG có flag tắt. Nếu ./proton_config/ rỗng hoặc VPN không kết nối được
  → script sẽ FAIL-FAST (sys.exit(1)), KHÔNG fallback về IP thật.
  (Bản này là bản sao vpn-only của youtube_researcher_youtube_subs_multi.py
   — để tắt VPN hãy dùng bản gốc và truyền --no-use-vpn.)

============================================================================
*** MULTI-INSTANCE SAFE VERSION ***
============================================================================
Bản sao của youtube_researcher_youtube_subs.py với các fix để chạy được
NHIỀU TIẾN TRÌNH SONG SONG với các đường dẫn file khác nhau mà không xung đột:

  + Cache (channel_id_cache, phase1_cache, video_details_cache) tách theo
    `--instance-id` / `--cache-dir` → KHÔNG đè JSON giữa các process.
  + Output folder timestamp có millisecond → KHÔNG đè transcriptions/audio
    folder khi 2 instance start cùng giây.
  + Log file có suffix instance_id → KHÔNG đè log TXT giữa các process.
  + VPN tunnel cô lập (`--vpn-isolated`): mỗi instance có openvpn PID riêng,
    log file riêng, KHÔNG dùng pkill để tránh kill nhầm tunnel instance khác.

CÁCH DÙNG (chạy song song 2 instance):
    # Terminal 1
    python youtube_researcher_youtube_subs_multi.py \\
        --channels-file ./channels_audio/channels_khoa_hoc_6.txt \\
        --output ./datasets_inst1 \\
    ...
    # Terminal 2 (cùng lúc)
    python youtube_researcher_youtube_subs_multi.py \\
        --channels-file ./channels_audio/channels_thoi_su_0.txt \\
        --output ./datasets_inst2 \\
        --use-vpn --vpn-isolated \\
        --instance-id inst2 \\
        --video-delay 5 --skip-existing
...
Usage:
    ...
    # Mode 1 kênh
    python youtube_researcher_youtube_subs_multi.py --channel "https://www.youtube.com/@vietnh1009"

    # Bật ProtonVPN để rotate IP (5 server free: CA/MX/NL/SG/US)
    python youtube_researcher_youtube_subs_multi.py --use-vpn

    # Rotate IP VPN mỗi 10 request (MỖI INSTANCE CÓ TUNNEL RIÊNG)
    python youtube_researcher_youtube_subs_multi.py --use-vpn --vpn-isolated --vpn-rotate-every 10

    # CYCLE 11: cứ 10 fake VPN thì 1 IP thật, lặp lại (reset rate-limit)
    python youtube_researcher_youtube_subs_multi.py --use-vpn --vpn-isolated \\
        --vpn-rotate-every 10 --vpn-real-ip-cycle 11
"""
...
class IsolatedVPNRotator:
    ...
    Fix 3 vấn đề của VPNRotator gốc khi chạy nhiều process cùng lúc:
      ...
      1. OPENVPN_LOG là constant module-level ("/tmp/openvpn-proton.log")
         → 2 instance ghi đè log của nhau.
         FIX: monkey-patch `vpn_rotator.OPENVPN_LOG` thành
              `/tmp/openvpn-proton-<instance_id>.log` TRƯỚC khi tạo inner rotator.

      2. `_disconnect()` có fallback `pkill -9 -u $UID -f openvpn.*proton_config`
         → kill nhầm tunnel của instance khác khi rotate.
         FIX: override `_disconnect()` để chỉ kill PID của mình (tracked qua
              `self._inner._current_pid`), BỎ pkill fallback.

      3. `_is_connected()` dùng `pgrep -f openvpn.*proton_config`
         → thấy openvpn process của instance khác → tưởng đã connected
           → skip connect → dùng IP thật.
         FIX: override `_is_connected()` để check trực tiếp PID của mình
              bằng `os.kill(pid, 0)` (không phụ thuộc pgrep).

    KHÔNG sửa `vpn_rotator.py` gốc → giữ backward-compat cho scripts khác
    đang dùng class gốc.

    Usage:
        rotator = IsolatedVPNRotator(
            instance_id="inst1",
            rotate_every=10,
            strategy="random",
        )
    ...
    Interface giống hệt VPNRotator: __len__, next(), mark_failed(),
    mark_success(), remove_proxy(), acquire(), print_stats(), ...
    Tất cả methods/attrs không bị override sẽ được forward qua __getattr__.
    """
    ...
    def __init__(self, instance_id: str, **vpn_kwargs):
        ...
        # === FIX 1: monkey-patch OPENVPN_LOG và OPENVPN_BIN path constants ===
        # Phải patch TRƯỚC khi tạo VPNRotator (vì _connect_server() dùng
        # OPENVPN_LOG ngay khi connect).
        import vpn_rotator as _vr_mod
        _vr_mod.OPENVPN_LOG = self._instance_log
        # Lưu reference để dùng trong các nested function (FIX NameError)
        self._vr_mod = _vr_mod

        # === Tạo inner rotator ===
        self._inner = VPNRotator(**vpn_kwargs)

        # Override pid_file path: wrap _connect_server để dùng prefix riêng
        self._patch_connect_server_pid()

    def _patch_connect_server_pid(self):
        """
        Wrap _connect_server để tạo pid_file theo instance_id,
        tránh 2 instance ghi đè `/tmp/openvpn-proton.pid.<idx>.<retry>`.
        """
        ...
        def _patched(idx: int, retry: int = 0) -> bool:
            """Wrapper: ghi đè pid_file path trước khi gọi _connect_server gốc."""
            import subprocess as _sp
            import time as _t

            ovpn = self._inner._ovpn_files[idx]
            log = _vr_log = __import__("logging").getLogger("vpn_rotator")
            log.info("VPN[isolated=%s]: connecting to %s (attempt %d)",
            ...
            new_pid = None
            try:
                # Log file RIÊNG cho instance này
                log_path = f"{self._instance_log}.{idx}.{retry}"
                # PID file RIÊNG cho instance này (FIX 6 từ analysis)
                ...
                try:
                    ...
                    log.warning("VPN[isolated=%s]: openvpn parent không thoát sau 10s, kill",
                                self.instance_id)
                    proc.kill()
                    proc.wait()
                    return False
                ...
                for _ in range(20):
                    ...
                    if pid_file.exists():
                        try:
                            ...
                            pass
                if new_pid is None:
                    log.warning("VPN[isolated=%s]: openvpn start nhưng không tạo pid file",
                                self.instance_id)
                    return False

                if old_pid is not None and new_pid == old_pid:
                    log.warning("VPN[isolated=%s]: PID mới == PID cũ (%s) — retry",
                                self.instance_id, new_pid)
                    try:
                        os.kill(new_pid, 9)
                    except Exception:
                        pass
                ...
                log.info("VPN[isolated=%s]: started openvpn PID=%s, file=%s",
                         self.instance_id, new_pid, ovpn.name)
            except Exception as e:
                log.error("VPN[isolated=%s]: không start được openvpn: %s",
                          self.instance_id, e)
                return False
            ...
            for i in range(self._vr_mod.CONNECT_TIMEOUT):
                ...
                if self._inner._has_tun0():
                    ...
                    if ip and ip != real_ip:
                        ...
                        self._inner._usage_count[idx] = (
                            self._inner._usage_count.get(idx, 0) + 1
                        )
                        self._inner._request_count = 0
                        self._inner._last_connect_time = _t.time()
                        log.info(
                            "VPN[isolated=%s]: ✓ connected to %s, IP=%s, PID=%s "
                            "(sau %ds, attempt %d)",
                            self.instance_id, ovpn.name, ip, new_pid, i + 1, retry + 1,
                        )
                        return True
                elif i >= 3:
                    ip = self._inner._get_current_ip()
                    ...
                    if ip and real_ip and ip != real_ip:
                        ...
                        self._inner._last_connect_time = _t.time()
                        log.info(
                            "VPN[isolated=%s]: ✓ connected (no tun0) to %s, IP=%s, "
                            "PID=%s (sau %ds, attempt %d)",
                            self.instance_id, ovpn.name, ip, new_pid, i + 1, retry + 1,
                        )
                        return True

            log.warning("VPN[isolated=%s]: timeout, tunnel không lên tới %s",
                        self.instance_id, ovpn.name)
            self._inner._disconnect()
            return False
    ...
    # === FIX 2: Override _is_connected() — KHÔNG dùng pgrep ===
    def _is_connected(self) -> bool:
        """Check tunnel của MÌNH còn sống không (check trực tiếp PID).

        KHÔNG dùng `pgrep -f openvpn.*proton_config` vì sẽ thấy process
        của instance khác (vd: instance 2 đang connect CA-free-14) → tưởng
        mình đã connected → skip connect → dùng IP thật.
        """
        pid = getattr(self._inner, "_current_pid", None)
        if pid is None:
            return False
        try:
            os.kill(pid, 0)  # signal 0 chỉ check, không kill
            return True
        except (ProcessLookupError, PermissionError):
            return False

    # === FIX 3: Override _disconnect() — KHÔNG dùng pkill ===
    def _disconnect(self):
        """Disconnect tunnel của MÌNH (kill chỉ PID của mình).

        KHÔNG gọi `pkill -9 -u $UID -f openvpn.*proton_config` vì sẽ kill
        nhầm tunnel của instance khác đang chạy song song.
        """
        pid = getattr(self._inner, "_current_pid", None)
        if pid is None:
            return
        ...
        try:
            ...
            # Process thuộc user khác → KHÔNG fallback pkill (vì sẽ kill nhầm
            # process của instance khác). Bỏ qua.
            import logging as _logging
            _logging.getLogger("vpn_rotator").warning(
                "VPN[isolated=%s]: PID %s thuộc user khác, KHÔNG pkill để "
                "tránh kill nhầm instance khác",
                self.instance_id, pid,
            )
            return

        # Bước 2: chờ process chết, check mỗi 0.5s, tối đa 10s
        import time as _t
        deadline = _t.time() + 10
        while _t.time() < deadline:
            try:
                ...
                self._inner._current_pid = None
                _t.sleep(2)  # ROTATE_DELAY
                return
            except PermissionError:
                pass

        # Bước 3: timeout → SIGKILL (CHỈ PID của mình)
        try:
            os.kill(pid, 9)
            _t.sleep(0.5)
        except ProcessLookupError:
    ...
    # === Forward các methods/attrs của VPNRotator qua __getattr__ ===
    def __getattr__(self, name):
        # Tránh recursion khi self._inner chưa được set (vd khi __init__ fail)
        if name in ("_inner", "instance_id", "_instance_log", "_instance_pid_prefix"):
            raise AttributeError(name)
        return getattr(self._inner, name)

    # === Explicit __len__ / __bool__ để khớp interface ===
    ...
    # === Explicit cleanup ===
    def disconnect(self):
        """Disconnect tunnel của mình (cleanup khi kết thúc)."""
        self._disconnect()
...
def get_isolated_vpn_rotator_from_config(
    ...
    """
    Tạo IsolatedVPNRotator. Trả về None nếu không có file .ovpn
    (để caller fallback sang proxy thường / IP thật).

    Args:
        real_ip_cycle: chu kỳ cycle "N fake VPN → 1 IP thật".
            0 (mặc định) = TẮT cycle (chỉ fake IP).
            N > 0 = cứ N request thì có 1 IP thật (default route, VPN disconnect).
    """
    try:
        ...
        import logging as _logging
        _logging.getLogger("vpn_rotator").warning(
            "IsolatedVPN rotator không khả dụng: %s", e
        )
        return None
...
class VideoCandidate:
    ...
    subtitles: dict = field(default_factory=dict)          # Manual subtitles URLs (Phase 2 yt-dlp cache)
...
def _instance_root() -> Path:
    """
    Folder riêng cho mỗi instance (multi-instance safe).

    Thứ tự ưu tiên:
      1. args.cache_dir (nếu user truyền --cache-dir, dùng path đó)
      2. <script_dir>/.cache_<INSTANCE_ID>/ (tự tạo theo instance_id)

    Tất cả cache file/folder (channel_id_cache, phase1_cache, video_details_cache,
    cookies copies, ...) đều phải nằm trong folder này để tránh đè giữa các
    process khi chạy song song.

    Raises:
        RuntimeError: nếu main() chưa set INSTANCE_ID (gọi từ nơi khác ngoài main).
    """
    # args là global từ argparse; nếu chưa có thì dùng PID (best-effort)
    try:
    ...
    inst_id = globals().get("INSTANCE_ID")
    if not inst_id:
        # Fallback: dùng PID làm instance_id (an toàn cho single-process)
        inst_id = f"pid{os.getpid()}"
    return Path(__file__).parent / f".cache_{inst_id}"


def _channel_id_cache_path() -> Path:
    """Path luu cache channel_id theo handle/URL (tang toc resolve_channel_id).

    SHARED across instances: dùng folder chung <script_dir>/.cache_shared/
    thay vì tách theo INSTANCE_ID. Lý do:
      - resolve_channel_id() chạy đầu tiên cho MỖI channel; nếu mỗi instance
        có cache riêng thì 2 instance chạy song song đều phải gọi API/yt-dlp
        cho cùng 1 channel → lãng phí.
      - Channel handle/URL là identity ổn định (không phụ thuộc run), nên
        share cache giữa các instance/run là an toàn.

    Phase1 cache và video_details cache VẪN tách theo instance (vì chứa
    data phụ thuộc run-time, vd: timestamp fetched_at).
    """
...
def resolve_channel_id(api_key: str, channel_input: str,
    ...
    Thứ tự ưu tiên (mỗi bước nhanh hơn bước sau):
      1. Direct channel ID (UCxxx)         → return ngay
      2. /channel/UCxxx từ URL             → return ngay (không gọi API)
      3. Shared cache hit                   → return ngay (~0ms)
      4. YouTube Data API (forHandle/forUsername) → ~0.5-1s, 50 units
      5. yt-dlp fallback                    → 5-20s (chỉ khi API hết quota
                                               hoặc URL không match pattern)
    ...
    """
    # === BƯỚC 1: Direct channel ID ===
    if channel_input.startswith("UC") and len(channel_input) == 24:
        return channel_input

    # Extract clean URL (bỏ trailing slash)
    url = channel_input.strip().rstrip("/")

    # === BƯỚC 2: /channel/UCxxxxx: trích trực tiếp từ URL (không cần gọi API) ===
    ...
    # === BƯỚC 3: Cache hit (shared cross-instance) ===
    cache = _load_channel_id_cache()
    if channel_input in cache:
        return cache[channel_input]

    # === BƯỚC 4: FAST PATH — YouTube Data API (nếu có key) ===
    # Nhanh hơn yt-dlp ~10-30x: ~0.3-0.5s vs 5-20s
    # Thử NHIỀU pattern URL để cover hết cases
    if _YOUTUBE_API_KEYS:
        import requests as _req

        # Detect tất cả pattern từ URL
        ...
        # Bare @handle (không có URL prefix): vd "@vietnh1009" hoặc "vietnh1009"
        ...
        if not handle_match and not custom_match and not user_match:
            if url.startswith("@"):
                ...
                # Pure handle như "vietnh1009" (không có scheme/host)
                bare_handle_match = re.match(r"([\w.-]+)", url)

        try:
            for api_key in _YOUTUBE_API_KEYS:
                # Ưu tiên forHandle (works với @handle mới) trước
                if handle_match:
                    resp = _req.get(
                    ...
                    resp = _req.get(
                        "https://www.googleapis.com/youtube/v3/channels",
                        params={"key": api_key, "part": "id",
                                "forUsername": custom_match.group(1)},
                        timeout=10,
                    )
                elif user_match:
                    resp = _req.get(
                    ...
                    # Bare @handle (không có URL) → thử forHandle
                    resp = _req.get(
                        "https://www.googleapis.com/youtube/v3/channels",
                        params={"key": api_key, "part": "id",
    ...
    # === BƯỚC 5: FALLBACK — yt-dlp (5-20s, chậm nhưng work cho mọi URL) ===
    try:
        import yt_dlp
    except ImportError:
    ...
    # Dùng /videos để đảm bảo lấy channel (không phải playlist)
    ...
    for attempt in range(1, 3):
        try:
            ...
            if is_blocked and attempt < 2:
                print(f"  [yt-dlp] resolve_channel_id attempt {attempt}/2 blocked, retry in 5s...")
                time.sleep(5)
                continue
            break  # non-block error → fallback xu ly duoi
    if info:
        ...
        if channel_id and channel_id.startswith("UC") and len(channel_id) == 24:
            cache[channel_input] = channel_id
            _save_channel_id_cache(cache)
            return channel_id
        ...
        return result_id
    if last_err:
        print(f"  [yt-dlp] resolve_channel_id error: {last_err}")
...
def fetch_video_info_via_ytdlp(video_id: str,
    ...
    Co retry don gian (2 lan) + log neu gap captcha/429.
    """
    try:
        import yt_dlp
    except ImportError:
    ...
    for attempt in range(1, 3):
        try:
            ...
            if is_blocked and attempt < 2:
                print(f"  [yt-dlp] video {video_id} attempt {attempt}/2 blocked, retry in 5s...")
                time.sleep(5)
                continue
            # Non-block error → log và trả None
            print(f"  [yt-dlp] video {video_id} error: {e}")
            return None
    if last_err:
        print(f"  [yt-dlp] video {video_id} failed after 2 attempts: {last_err}")
    return None
...
class YouTubeResearcher:
    ...
    def __init__(self, api_key: str, output_dir: str = "./researched_videos",
        ...
        # === MULTI-STAGE PROXY: rotator riêng cho audio download ===
        # Khi proxy_mode='split': _audio_rotator chỉ được dùng khi IP thật fail
        # Khi proxy_mode='auto'/'always': _audio_rotator = _rotator (cùng 1 rotator)
        self._audio_rotator = audio_proxy_rotator if audio_proxy_rotator is not None else proxy_rotator
        self._proxy_mode = proxy_mode
        self._direct_blocked = False
        # Track đã escalate sang audio_rotator cho audio chưa (chỉ trong split mode)
        self._audio_escalated = False

    def _next_proxy(self) -> tuple:
        """
        Lấy proxy URL tiếp theo từ rotator (hoặc None nếu không có rotator).

        Returns:
            tuple (proxy_url, use_real_ip):
              - proxy_url: str URL proxy (vd "http://1.2.3.4:8080") hoặc None
                (None = dùng default route. Nếu VPN đang connect thì default route
                đã qua tunnel. Nếu không VPN thì default route = IP thật).
              - use_real_ip: bool
                + True  → request hiện tại NÊN dùng IP thật (VPN đã disconnect
                          theo cycle "N fake → 1 real"). Caller KHÔNG acquire tunnel.
                + False → dùng VPN tunnel như bình thường (hoặc không có VPN).
        """
        if not self._rotator:
            return (None, False)

        # Trigger next() để update state (count, rotate, cycle)
        url = self._rotator.next()

        # Check cycle mode SAU khi next() xử lý state
        # next() đã SET _use_real_ip=True ở request thứ N (cuối cycle fake)
        # và reset _use_real_ip=False ở request thứ N+1 (IP thật) → request này
        # là request kế tiếp (N+2), reconnect VPN.
        # → is_using_real_ip() chỉ True ở 1 request duy nhất giữa cycle.
        is_real = (
            hasattr(self._rotator, 'is_using_real_ip')
            and callable(getattr(self._rotator, 'is_using_real_ip', None))
            and self._rotator.is_using_real_ip()
        )
        ...
        if is_real:
            print(f"    [proxy] → IP thật (cycle mode: VPN đã disconnect)")
            return (None, True)  # None proxy, dùng default route = IP thật
    ...
    def _proxy_guard(self):
        ...
        Nếu đang ở phase "IP thật" (cycle mode) → return dummy (không acquire
        vì không có tunnel để bảo vệ).
        """
        # === Dummy no-op context (reusable) ===
        class _NoOp:
        ...
        # Nếu VPN đang ở phase "IP thật" (đã disconnect theo cycle),
        # KHÔNG acquire tunnel (vì tunnel không tồn tại).
        try:
            if (
                hasattr(self._rotator, 'is_using_real_ip')
                and callable(getattr(self._rotator, 'is_using_real_ip', None))
    ...
    def _guard_and_proxy(self):
        ...
        Dùng cho pattern:
            with self._guard_and_proxy() as gp:
                proxy_url = gp.proxy_url
                use_real_ip = gp.use_real_ip
                # proxy_url có thể None (dùng default route qua tunnel HOẶC IP thật)
                # use_real_ip = True nếu đang ở phase IP thật (cycle mode)
                ...
        """
        ...
        class _GuardProxy:
            def __init__(self, g, owner):
                ...
                self._owner = owner
                self.proxy_url = None
                self.use_real_ip = False
            def __enter__(self):
                self._g.__enter__()
                # _next_proxy() trả tuple (proxy_url, use_real_ip)
                self.proxy_url, self.use_real_ip = self._owner._next_proxy()
                return self
            def __exit__(self, *a):
    ...
    def _proxy_for_fallback(self) -> Optional[str]:
        ...
        Lưu ý cycle mode: khi VPN ở phase "IP thật", _next_proxy() trả
        (None, True) → method này trả None (như đang ở direct). Caller
        nào cần phân biệt nên gọi _next_proxy() trực tiếp.
        """
        if self._direct_blocked and self._rotator and len(self._rotator) > 0:
            proxy, _use_real = self._next_proxy()
            if proxy:
                return proxy
    ...
    def _audio_proxy_for_dl(self, dl_attempt: int = 1) -> Optional[str]:
        """Lấy proxy cho audio download theo --proxy-mode.

        Mode 'auto'/'always': trả proxy từ _rotator (giống cũ)
        Mode 'split':        MẶC ĐỊNH trả None (IP thật, tải nhanh)
                             CHỈ trả proxy từ _audio_rotator khi:
                             - dl_attempt > 1 (IP thật fail lần đầu)
                             - Hoặc _direct_blocked đã được set
        Mode 'never':        LUÔN trả None

        QUAN TRỌNG: Khi _audio_rotator là VPNRotator (system tunnel), next() trả None
        NHƯNG tunnel vẫn active → set _use_vpn_tunnel=True để caller biết dùng
        default route (đã đi qua tunnel). KHÔNG phải fallback về IP thật.
        """
        if self._proxy_mode == "never":
            self._use_vpn_tunnel = False
            return None

        if self._proxy_mode == "split":
            # Mặc định: dùng IP thật (None) để tận dụng băng thông
            # Chỉ escalate sang audio_rotator khi IP thật bị block
            if dl_attempt > 1 or self._direct_blocked:
                if self._audio_rotator and len(self._audio_rotator) > 0:
                    # Trigger VPN connect/rotate nếu cần
                    self._audio_rotator.next()
                    # Kiểm tra VPN có thật sự active không
                    is_vpn_active = self._is_vpn_active(self._audio_rotator)
                    if is_vpn_active:
                        self._audio_escalated = True
                        self._use_vpn_tunnel = True
                        print(f"  [audio-dl] Escalate sang VPN tunnel (IP that bi block) "
                              f"via {self._short_proxy_vpn(self._audio_rotator)}")
                        return None  # None = dùng default route qua VPN tunnel
            # Không escalate hoặc VPN chưa active → dùng IP thật
            self._use_vpn_tunnel = False
            return None

        # 'auto' hoặc 'always': dùng rotator như cũ
        self._use_vpn_tunnel = False
        return self._proxy_for_fallback()

    def _is_vpn_active(self, rotator) -> bool:
        """
        Check xem VPN rotarator có thật sự có tunnel active không.

        VPNRotator (và IsolatedVPNRotator) lưu _current_pid khi tunnel lên,
        _current_idx khi connected thành công. Check 2 thuộc tính này.
        """
        if rotator is None:
            return False
        # VPNRotator có _current_idx; IsolatedVPNRotator forward qua __getattr__
        current_idx = getattr(rotator, "_current_idx", None)
        current_pid = getattr(rotator, "_current_pid", None)
        if current_idx is not None and current_pid is not None:
            # Verify PID còn sống
            try:
                import os
    ...
    def _short_proxy_vpn(self, rotator) -> str:
        """Trả về short label cho VPN rotarator (vd 'CA-14' thay vì url dài)."""
        try:
            inner = getattr(rotator, "_inner", rotator)
            idx = getattr(inner, "_current_idx", None)
            if idx is not None:
                ovpn = inner._ovpn_files[idx]
                return f"VPN-{ovpn.stem.split('.')[0]}"
        except Exception:
            pass
        return "VPN"
    ...
    # === KEYWORDS: YouTube IP-block / captcha / rate-limit signals ===
    # Cập nhật 2026-06: thêm captcha/challenge (sau khi gặp lỗi
    # "YouTube is requiring a captcha challenge before playback" — IP datacenter
    # bị Google flag nhưng yt-dlp KHÔNG raise error có chứa "sign in" / "bot").
    _YT_BLOCKED_KEYWORDS = (
        # Rate-limit / quota
        ...
        'captcha', 'challenge',  # ← thêm 2026-06-18: fix bug escalation
        # Network-level
    ...
    @classmethod
    def _is_youtube_blocked_error(cls, err) -> bool:
        """
        Phát hiện lỗi do YouTube chặn IP / rate-limit / captcha challenge.

        Args:
            err: Exception hoặc string error message

        Returns:
            True nếu lỗi thuộc loại cần escalate (đổi proxy / backoff).
            False nếu là lỗi khác (video unavailable, format không có, ...).

        Note:
            Dùng _YT_BLOCKED_KEYWORDS làm canonical source. Nếu thêm keyword
            mới, CHỈ sửa list này — các chỗ retry/escalate sẽ tự động cover.
        """
        err_str = str(err).lower()
        return any(k in err_str for k in cls._YT_BLOCKED_KEYWORDS)

    def _on_youtube_blocked(self, err, proxy_url: Optional[str], context: str):
        """
        Handler thống nhất khi gặp lỗi YouTube chặn IP:
        - Nếu đang dùng IP thật (no proxy) → escalate sang VPN
        - Nếu đang dùng proxy → phân loại: dead (xóa vĩnh viễn) vs rate-limit (cooldown)

        Args:
            err: Exception gốc
            proxy_url: proxy hiện tại (None = đang dùng IP thật)
            context: mô tả ngắn cho log (vd: 'phase1', 'audio-dl', 'subs')
        """
        if not proxy_url:
            # Đang dùng IP thật → escalate sang VPN ngay
            self._escalate_to_proxy()
            return

        # Đang dùng proxy → phân loại dead vs rate-limit
    ...
    def _safe_extract_info(self, url: str, ydl_opts: dict, max_attempts: int = 3,
                           context: str = "extract", extra_cookies: str = None):
        """
        Wrapper an toàn cho yt-dlp extract_info() có retry + escalate proxy.

        Args:
            url: URL cần extract (channel URL, video URL, playlist URL, ...)
            ydl_opts: dict options cho yt-dlp (sẽ KHÔNG bị mutate)
            max_attempts: số lần retry tối đa
            context: mô tả ngắn cho log (vd: 'phase1', 'channel-resolve', 'comments')
            extra_cookies: path tới cookies file riêng (optional, vd: per-worker cookies)

        Returns:
            info dict từ yt-dlp, hoặc None nếu tất cả attempts fail.

        Behavior:
            - Attempt 1: dùng IP thật / proxy hiện tại (theo --proxy-mode)
            - Attempt 2+: nếu gặp captcha/429/block → escalate sang VPN (nếu có)
            - Mỗi attempt sleep 3s, 6s, 9s (backoff đơn giản)
            - Proxy / IP bị fail sẽ được mark → tránh dùng lại
        """
        import yt_dlp

        # Deep copy opts để không mutate caller's dict
        opts_base = dict(ydl_opts)
        if extra_cookies and "cookiefile" not in opts_base:
            opts_base["cookiefile"] = extra_cookies

        last_err = None
        for attempt in range(1, max_attempts + 1):
            # Lấy proxy theo policy hiện tại
            if self._rotator:
                proxy_url, _use_real = self._next_proxy()
            else:
                proxy_url = None
            opts = dict(opts_base)
            if proxy_url:
            ...
            try:
                ...
                # Success → mark success để tránh rotate không cần thiết
                ...
                if is_blocked and attempt < max_attempts:
                    # Có dấu hiệu captcha/429/block → escalate sang VPN nếu có
                    if not self._rotator:
                        # Không có rotator → chỉ log, sleep, retry với IP thật
                        print(f"    [{context}] attempt {attempt}/{max_attempts} blocked "
                              f"(no rotator available): {err_short}")
                    else:
                        self._on_youtube_blocked(e, proxy_url, context=context)
                        print(f"    [{context}] attempt {attempt}/{max_attempts} blocked, "
                              f"switching to {'proxy' if not proxy_url else 'next proxy'}, "
                              f"retry in {3 * attempt}s...")
                    time.sleep(3 * attempt)
                    continue
                # Không phải block, hoặc hết retry → trả None
                if attempt == max_attempts:
                    print(f"    [{context}] attempt {attempt}/{max_attempts} fail "
                          f"(non-block error or exhausted): {err_short}")
                break
        return None
    ...
    def _apply_auth_skip(ydl_opts: dict) -> dict:
        ...
        #    Phải trỏ tới node >= 23.5.0 (yt-dlp yêu cầu) — nvm node 24 thỏa mãn,
        #    node 18 mặc định trên hệ thống sẽ bị báo "unavailable" và challenge solver
        #    bị tắt, dẫn đến "Sign in to confirm you're not a bot".
        if "js_runtimes" not in ydl_opts:
            node_path = "/home/sotatek/.nvm/versions/node/v24.15.0/bin/node"
            if not Path(node_path).exists():
                # fallback: dùng node đầu tiên trong PATH
                import shutil as _shutil
                node_path = _shutil.which("node") or "node"
            ydl_opts["js_runtimes"] = {"node": {"path": node_path}}

        # 4. Chỉ định rõ base_url cho plugin bgutil:http (mặc định là
        #    http://127.0.0.1:4416, set thêm để chắc chắn đúng server đang chạy)
        ydl_opts["extractor_args"].setdefault("youtubepot-bgutilhttp", {})
        bg_http_args = ydl_opts["extractor_args"]["youtubepot-bgutilhttp"]
        if "base_url" not in bg_http_args:
            bg_http_args["base_url"] = ["http://127.0.0.1:4416"]
    ...
    def _cleanup_orphan_part_files(self, audio_dir: Path, min_size_mb: int = 100) -> int:
        """
        Xóa orphan .part/.ytdl files (còn lại sau khi crawler bị kill giữa chừng).

        Logic:
        - Quét audio_dir/{timestamp}/*.part và *.ytdl
        - Nếu KHÔNG có file .wav tương ứng (cùng stem) → orphan
        - Chỉ xóa nếu size > min_size_mb (tránh xóa file đang được tải bởi
          instance khác / worker thread khác trong cùng batch)
        ...
        # Tập stem của các file .wav đang tồn tại → orphan = .part không có .wav
        wav_stems = {p.stem for p in audio_dir.glob("*.wav")}

        # Quét cả .part và .ytdl
        for part_file in list(audio_dir.glob("*.part")) + list(audio_dir.glob("*.ytdl")):
            try:
            ...
            if size < min_size_bytes:
                # File quá nhỏ - có thể đang được tải bởi instance khác, skip
                continue

            # Lấy stem gốc (bỏ .part-FragNNN hoặc .ytdl suffix)
            # VD: "abc.mp4.part-Frag510.part" → stem gốc = "abc.mp4"
            #     "abc.mp4.part" → stem gốc = "abc.mp4"
            #     "abc.mp4.ytdl" → stem gốc = "abc.mp4"
            ...
            for suffix in (".part-Frag", ".part", ".ytdl"):
                if suffix in original_stem:
                    ...
                    break
            original_stem = original_stem.rsplit(".", 1)[0]  # bỏ extension .mp4/.webm

            # Nếu đã có .wav tương ứng → KHÔNG xóa (an toàn)
            ...
            try:
                ...
                print(f"  [CLEANUP] Xóa orphan {part_file.name} ({size_mb:.1f}MB) - không có .wav tương ứng")
                part_file.unlink()
                deleted += 1
                deleted_total_size += size
            except OSError as e:
                print(f"  [CLEANUP] Không xóa được {part_file.name}: {e}")

        if deleted > 0:
            print(f"  [CLEANUP] Đã xóa {deleted} orphan .part files "
                  f"({deleted_total_size / (1024**3):.2f} GB)")
        return deleted
    ...
    #
    # Đặt cùng folder với transcriptions/{timestamp}/ để mỗi run có marker riêng.
    # Nếu user chạy lại với --force-retranscribe, marker sẽ KHÔNG được check
    # (vì skip_existing=False sẽ bypass _partition check).
    ...
    def _phase1_cache_path() -> Path:
        ...
        MULTI-INSTANCE: lưu trong folder riêng của instance.
        """
        return _instance_root() / "phase1_cache"
    ...
    def _video_details_cache_path() -> Path:
        ...
        MULTI-INSTANCE: lưu trong folder riêng của instance.
        """
        return _instance_root() / "video_details_cache"
    ...
    def _phase1_via_playlist(
        ...
        # ====== BƯỚC 0: Load cache Phase 1 (skip API nếu cache hit) ======
        # Cache chứa list video_ids từ lần fetch trước (TTL 30 ngày).
        # Nếu cache HIT → return ngay, skip gọi playlistItems.list API (~21-30s).
        cached_video_ids = self._load_phase1_cache(channel_id, ttl_days=30)
        if cached_video_ids:
            print(f"  [Phase 1 PL] Cache HIT: skip playlistItems.list, "
                  f"dùng {len(cached_video_ids)} video IDs từ cache (TTL 30 ngày)")
            return cached_video_ids[:max_results], False
        ...
        # Lưu cache Phase 1 (sau khi playlistItems.list thành công)
        # để lần chạy sau có thể skip API call.
        if all_video_ids and not quota_exhausted:
    ...
    def _early_filter_videos(self, videos: list, min_duration: int = 600,
        ...
        KHÔNG lọc theo transcript (cần gọi yt-dlp extract_info tốn time).
    ...
    def fetch_channel_videos(
        ...
        for attempt in range(1, phase1_max_retries + 1):
            ...
            try:
                ...
                is_blocked = self._is_youtube_blocked_error(e)
                if is_blocked:
                    self._on_youtube_blocked(e, listing_proxy, context="phase1")
                if attempt < phase1_max_retries:
                    listing_proxy, _use_real = self._next_proxy()
                    if listing_proxy:
                        ydl_listing_opts["proxy"] = listing_proxy
    ...
    def _api_item_to_ytdlp_dict(self, item: dict) -> dict:
        ...
        return {
            ...
            "subtitles": {},
            "automatic_captions": {},
            # LƯU Ý: YouTube Data API field 'caption' (contentDetails.caption) CHỈ
            # cho biết có MANUAL caption (do uploader upload), KHÔNG bao gồm
            # auto-generated captions. Vì transcribe_with_youtube() download
            # auto-captions, nên field này KHÔNG đáng tin → KHÔNG dùng để skip.
            # (Đã test: video dhXVkt40XPM đã transcribe OK nhưng API caption=false.)
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    with self._proxy_guard():
                        ...
                        proxy, use_real_ip = self._next_proxy()
                        ydl_opts = {
                        ...
                        # Nếu use_real_ip=True: KHÔNG set proxy, traffic đi qua
                        # default route = IP thật (VPN đã disconnect theo cycle)
                        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                            info = ydl.extract_info(
                    ...
                    if is_blocked and attempt < max_retries:
                        # Captcha/429/block → escalate sang VPN nếu đang ở IP thật
                        # (sẽ trigger VPN connect hoặc rotate sang server khác)
                        self._on_youtube_blocked(e, proxy if 'proxy' in dir() else None,
                                                  context="phase2-extract")
                        if attempt < max_retries:
                            time.sleep(min(2 ** attempt, 8))
                            continue
                    # Không phải block, hoặc hết retry → sleep rồi exit
                    if attempt < max_retries:
                        time.sleep(min(2 ** attempt, 8))
            if last_err:
                print(f"    [phase2-extract] {video_id} failed after {max_retries} attempts: "
    ...
    def _build_video_from_ytdlp(self, info: dict) -> VideoCandidate:
        ...
        # LƯU Ý: YouTube Data API field 'caption' CHỈ cho biết có MANUAL caption,
        # không bao gồm auto-generated captions. Vì transcribe_with_youtube()
        # download auto-captions, không dùng info.get("caption_available")
        # (đã thử nhưng sai - skip cả video có auto-caps).
    ...
    def _fetch_top_comments(self, youtube, video_id, max_comments=5):
        ...
        Co retry + escalate proxy khi gap captcha/429.
        """
        ydl_opts = {
            "quiet": True,
            "no_warnings": True,
        ...
        info = self._safe_extract_info(
            f"https://www.youtube.com/watch?v={video_id}",
            ydl_opts, max_attempts=2, context="comments",
        )
        if not info:
            return []
    ...
    # Engine duy nhất là _get_youtube_transcript_via_ytdlp()
    # (gọi từ transcribe_with_youtube() với retry 2 lần qua proxy).
    ...
    def _get_youtube_transcript_via_ytdlp(self, video_id: str,
        ...
        if info_cached is not None:
            ...
            if not has_subs:
                # info_cached = {} → đã biết video này không có subs → return None
    ...
    def transcribe_with_youtube(self, video_id: str, audio_path: Path = None, lang: list = None,
        ...
        # Chi dung IP fake (ProtonVPN), nhung van retry toi da 2 lan de co co hoi
        # bypass khi proxy dau bi flag/captcha. info_cached chi dung o attempt dau,
        # cac attempt sau se goi yt-dlp extract_info() moi voi proxy moi.
        max_attempts = 2
        ...
        # FIX v2: KHÔNG load toàn bộ file WAV vào RAM (sf.read() đọc hết file chỉ để lấy duration).
        # Với audio 30 phút ở 16kHz mono WAV ~150 MB, audio 2 giờ ~600 MB → rất chậm + tốn RAM.
        # → Ưu tiên đọc header WAV bằng `wave` (instant, vài KB), fallback ffprobe nếu không phải WAV.
        audio_duration = 0.0
            ...
            try:
                ...
                if audio_duration <= 0:
                    try:
                        ...
                        pass
                # Cách 3: fallback cuối cùng → sf.read (chỉ đọc 1 frame đầu để lấy sr)
                if audio_duration <= 0:
                    try:
...
"""
    ...
    def process_videos_pipeline(
        ...
        # === AUTO-CLEAN orphan .part files (fix bug "kill giữa chừng") ===
        # Khi crawler bị kill trong lúc đang download, file .part (1-2 GB)
        # bị bỏ lại trong audio/{timestamp}/ mà KHÔNG có file .wav tương ứng.
        # Lần chạy sau sẽ thấy file .part tồn tại nhưng _partition coi như "không có audio"
        # (vì check .wav) → video bị đẩy vào Bucket C → tải lại từ đầu.
        # → Tốn bandwidth vô ích (đã tải được ~95% rồi).
        #
        # FIX: Trước khi partition, scan audio/{timestamp}/ và xóa .part/.ytdl
        # KHÔNG có .wav tương ứng. CHỈ xóa nếu size > 100MB (tránh xóa nhầm
        # file đang được tải bởi instance khác - concurrent_workers case).
        ...
        for i, (video, audio_path, audio_filename) in enumerate(bucket_b, 1):
            ...
            # Giống bản gốc youtube_researcher_youtube_subs.py:
            # GỌI THẲNG transcribe_with_youtube, KHÔNG pre-check has_youtube_subs().
            # Pre-check dựa vào video_obj.caption_available (từ yt-dlp cache Phase 2)
            # có thể SAI do cache thiếu field subtitles (cookies/VPN issues),
            # sẽ bỏ lỡ video thực sự có subs trên YouTube.
            # transcribe_with_youtube() tự kiểm tra và trả None nếu thật sự không có.
            #
            # FIX v2: truyền info_cached từ Phase 2 yt-dlp metadata (nếu có)
            # để tránh gọi yt-dlp extract_info() lần 2 → giảm rate limit "Sign in".
            # VideoCandidate đã cache 'subtitles' + 'automatic_captions' URLs từ Phase 2.
            info_cached_b: dict = {}
            try:
        ...
        for i, (video, target_name, target_filename) in enumerate(bucket_c, 1):
            ...
            try:
                import yt_dlp
                max_dl_retries = 12  # 1 IP thật (attempt 1) + 10 VPN fake (attempt 2-11) + 1 IP thật cycle (attempt 12)
                info = None
                ...
                for dl_attempt in range(1, max_dl_retries + 1):
                    # === MULTI-STAGE PROXY: chọn proxy theo --proxy-mode ===
                    # auto/always: dùng _rotator (giống cũ)
                    # split:       dùng IP thật (None) mặc định, escalate sang audio_rotator khi fail
                    dl_proxy = self._audio_proxy_for_dl(dl_attempt=dl_attempt)
                    use_vpn_tunnel = getattr(self, "_use_vpn_tunnel", False)
                    if dl_proxy:
                        print(f"  [audio-dl] Attempt {dl_attempt}: dùng VPN (proxy)")
                    elif use_vpn_tunnel:
                        print(f"  [audio-dl] Attempt {dl_attempt}: dùng VPN tunnel (system route)")
                    else:
                        print(f"  [audio-dl] Attempt {dl_attempt}: dùng IP thật (direct)")
                    ydl_opts = {
                    ...
                    # Khi dùng VPN tunnel (system route), KHÔNG set proxy để traffic
                    # đi qua default route → đã route qua tun0
                    # === LOW-SPEED MONITOR: tự rotate VPN khi speed < 1 MB/s trong 10s ===
                    # BẬT cho CẢ 3 trường hợp: VPN proxy / VPN tunnel / IP thật (direct).
                    # - VPN proxy / VPN tunnel: rotate sang VPN server khác (mark_failed)
                    # - IP thật (direct): low-speed → next attempt (chuyển sang VPN nếu
                    #   dl_attempt tiếp theo theo --proxy-mode, hoặc retry direct).
                    # Logic:
                    #   - Lần 1: IP thật (direct, default split mode)
                    #   - Nếu IP thật < 1 MB/s trong 10s → next attempt (dl_attempt 2)
                    #   - Lần 2-10: VPN fake (audio_rotator.next() trả proxy)
                    #   - Nếu VPN fake < 1 MB/s → mark_failed(rotator) → next VPN server
                    #   - Lần 11: IP thật (theo cycle mode của VPNRotator: 10 fake + 1 real)
                    #   - Nếu IP thật < 1 MB/s → next attempt (dl_attempt 12 → quay lại VPN)
                    if dl_proxy is not None or use_vpn_tunnel:
                        proxy_label = self._short_proxy(dl_proxy) if dl_proxy else "VPN-tunnel"
                        speed_monitor = _AudioSpeedMonitor(
                            proxy_label=proxy_label,
                            video_id=video.video_id,
                        )
                        ydl_opts["progress_hooks"] = [speed_monitor]
                    elif getattr(self, "_rotator", None) is not None:
                        # IP thật (dl_proxy=None, not use_vpn_tunnel) mà CÓ rotator
                        # → VẪN bật monitor để trigger next attempt nếu < 1 MB/s
                        proxy_label = "DIRECT"
                        speed_monitor = _AudioSpeedMonitor(
                            proxy_label=proxy_label,
                            video_id=video.video_id,
                        ...
                        ydl_opts["progress_hooks"] = [speed_monitor]
                    try:
                        # Khi dùng VPN tunnel (system route), wrap trong _proxy_guard()
                        # để VPN rotate KHÔNG kill tunnel giữa request.
                        if use_vpn_tunnel:
                            with self._proxy_guard():
                                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ...
                        is_low_speed = 'low-speed' in err_msg
                        # Dùng helper chung (đã bao gồm captcha/challenge, kể cả low-speed qua 'proxy')
                        is_rate_limit = self._is_youtube_blocked_error(dl_err) or is_low_speed
                        # === LOGIC MỚI: low-speed kích hoạt retry cho CẢ IP thật ===
                        # Trước đây: IP thật low-speed → raise (fail luôn).
                        # Bây giờ: IP thật low-speed → cũng next attempt (giống VPN).
                        # Cycle mode: sau 10 lần VPN fake → lần 11 là IP thật
                        #   → nếu IP thật cũng < 1 MB/s → next attempt (lần 12 quay lại VPN).
                        # Điều kiện: low-speed + còn retry (dl_attempt < max_dl_retries)
                        if is_low_speed and dl_attempt < max_dl_retries:
                            via_label = (self._short_proxy(dl_proxy) if dl_proxy
                                         else ("VPN-tunnel" if use_vpn_tunnel else "DIRECT"))
                            print(f"  [audio-dl] attempt {dl_attempt}/{max_dl_retries} "
                                  f"low-speed via {via_label} → next attempt")
                            # Nếu đang dùng VPN proxy → mark failed để rotator chọn server khác
                            if dl_proxy and self._rotator:
                                self._rotator.mark_failed(dl_proxy)
                            # Nếu đang dùng VPN tunnel (system route) → mark failed rotator
                            elif use_vpn_tunnel and self._audio_rotator:
                                self._audio_rotator.mark_failed(None)
                            # Nếu đang dùng IP thật (direct):
                            #   - Không có rotator nào để mark
                            #   - _audio_proxy_for_dl() ở lần retry kế tiếp sẽ tự
                            #     escalate sang VPN (split mode: dl_attempt > 1)
                            #   - Hoặc nếu đã ở proxy_mode='always'/'auto': _rotator.next()
                            #     sẽ trigger rotate VPN theo cycle
                            wait = 3 * dl_attempt
                            time.sleep(wait)
                            continue
                        # === Backwards-compat: nếu là lỗi rate-limit/429/block khác (không phải low-speed)
                        # và còn retry thì escalate như cũ
                        if is_rate_limit and not is_low_speed and dl_attempt < max_dl_retries:
                            # Nếu đang dùng VPN tunnel, truyền None cho dl_proxy nhưng
                            # _on_youtube_blocked vẫn escalate qua _direct_blocked=True.
                            # Nếu captcha vẫn xuất hiện khi đã qua tunnel → trigger rotate VPN
                            self._on_youtube_blocked(dl_err, dl_proxy, context="audio-dl")
                            # Nếu vẫn fail khi đã ở VPN tunnel → mark VPN fail để rotate
                            if use_vpn_tunnel and self._audio_rotator:
                                # Force rotate VPN sang server khác
                                self._audio_rotator.mark_failed(None)
                            wait = 3 * dl_attempt
                            via_label = (self._short_proxy(dl_proxy) if dl_proxy
                                         else ("VPN-tunnel" if use_vpn_tunnel else "DIRECT"))
                            print(f"  [audio-dl] attempt {dl_attempt}/{max_dl_retries} "
                                  f"blocked/429 via {via_label}, retry in {wait}s...")
                            time.sleep(wait)
                            continue
                        # Hết retry hoặc không phải rate-limit/low-speed → raise để caller xử lý
                        raise
                if info is None:
            ...
            # sẽ bỏ lỡ video thực sự có subs trên YouTube.
            # transcribe_with_youtube() tự kiểm tra và trả None nếu thật sự không có.
            # Vẫn truyền info_cache (nếu có) để TẬN DỤNG subs URLs đã extract khi download
            # → tránh gọi yt-dlp extract_info() lần 2.
            try:
                result = self.transcribe_with_youtube(
                    video_id=video.video_id,
                    audio_path=audio_path,
...
class _AudioSpeedMonitor:
    """
    Hook vào yt-dlp `progress_hooks` để detect VPN server chậm và trigger rotate.

    Logic:
      - Mỗi lần yt-dlp callback (~mỗi ~1s khi đang download), lưu
        `(timestamp, downloaded_bytes)` vào self.samples.
      - Khi đủ ≥LOW_SPEED_MIN_SAMPLES sample trong LOW_SPEED_WINDOW_S giây → tính mean.
      - Nếu mean < LOW_SPEED_THRESHOLD_BPS → raise DownloadError("[low-speed] ...")
        để abort download. Existing exception handler sẽ bắt và trigger rotate
        sang server VPN khác.

    Chỉ dùng khi VPN được bật VÀ đang dùng proxy (`dl_proxy is not None`).
    IP thật (direct, split mode default) sẽ KHÔNG được monitor — đó là kênh
    download chính, nếu chậm sẽ được escalate sang audio_rotator qua rate-limit
    retry path hiện có.
    """
    LOW_SPEED_THRESHOLD_BPS = 1_000_000  # 1 MB/s = 1000 KB/s
    LOW_SPEED_MIN_SAMPLES = 10         # số sample tối thiểu
    LOW_SPEED_WINDOW_S = 10.0          # trong 10s đầu kể từ khi bắt đầu download

    def __init__(self, proxy_label: str, video_id: str):
        self.proxy_label = proxy_label
        self.video_id = video_id
        self.samples: list = []  # list[(timestamp, downloaded_bytes)]
        self.triggered = False
        self._last_log_t = 0.0
        # Lưu start time để chỉ check trong window 10s đầu
        self._start_t: float = 0.0

    def __call__(self, d: dict):
        # Chỉ check khi đang download (status="downloading")
        if d.get("status") != "downloading":
            return
        # Hook chỉ chạy khi có downloaded_bytes
        ...
        # Chỉ check trong window 10s đầu (sau đó để cho download tự nhiên)
        elapsed_since_start = now - self._start_t

        # Log thỉnh thoảng (mỗi 2s) để debug
        if now - self._last_log_t >= 2.0 and len(self.samples) >= 2:
            first_t, first_b = self.samples[0]
            last_t, last_b = self.samples[-1]
            dt = last_t - first_t
            db = last_b - first_b
            speed_bps = db / dt if dt > 0 else 0
            print(
                f"    [speed-monitor] video={self.video_id} "
                f"proxy={self.proxy_label} "
                f"sample={len(self.samples)}/{self.LOW_SPEED_MIN_SAMPLES}, "
                f"avg_speed={speed_bps/1000:.1f} KB/s, "
                f"threshold={self.LOW_SPEED_THRESHOLD_BPS/1000:.1f} KB/s"
            )
            self._last_log_t = now

        if self.triggered:
            return
        if len(self.samples) < self.LOW_SPEED_MIN_SAMPLES:
            return
        # Chỉ check trong window 10s đầu (sau đó không trigger dù speed bao nhiêu)
        if elapsed_since_start > self.LOW_SPEED_WINDOW_S:
            return

        # Tính mean trong window 10s gần nhất
        window_start = now - self.LOW_SPEED_WINDOW_S
        recent = [s for s in self.samples if s[0] >= window_start]
        if len(recent) < self.LOW_SPEED_MIN_SAMPLES:
            return
        first_t, first_b = recent[0]
        last_t, last_b = recent[-1]
        dt = last_t - first_t
        if dt <= 0:
            return
        db = last_b - first_b
        avg_bps = db / dt
        if avg_bps < self.LOW_SPEED_THRESHOLD_BPS:
            self.triggered = True
            try:
                ...
                return
            raise DownloadError(
                f"[low-speed] average speed {avg_bps/1000:.1f} KB/s "
                f"< threshold {self.LOW_SPEED_THRESHOLD_BPS/1000:.1f} KB/s "
                f"qua {self.proxy_label} ({len(recent)} samples in {dt:.1f}s) — "
                f"sẽ rotate sang IP khác"
            )
...
def parse_args():
    ...
    # DEFAULT: --use-vpn BẬT, --vpn-isolated BẬT, --proxy-mode split (an toàn multi-instance)
    # Opt-out: truyền --no-use-vpn / --no-vpn-isolated / --proxy-mode=auto|always|never
    # ===== VPN ROTATOR (ProtonVPN) — LUÔN BẬT trong bản này, không thể tắt =====
    # VPN là BẮT BUỘC. Flag --use-vpn chỉ tồn tại để tương thích với code phía dưới
    # (nhiều chỗ check args.use_vpn). Mặc định True và KHÔNG có flag tắt.
    # Để chạy không VPN hãy dùng bản gốc youtube_researcher_youtube_subs_multi.py
    # với --no-use-vpn.
    p.add_argument("--use-vpn", action="store_true", default=True,
                   help="(INTERNAL — LUÔN BẬT trong bản này, không thể tắt. "
                        "Để tắt VPN dùng file gốc youtube_researcher_youtube_subs_multi.py "
                        "và truyền --no-use-vpn.)")
    p.add_argument("--vpn-rotate-every", type=int, default=1000,
    ...
    p.add_argument("--vpn-real-ip-cycle", type=int, default=11,
                   help="CYCLE 'N fake VPN → 1 IP thật' (mặc định: 0 = TẮT). "
                        "Sau N request fake VPN thì request kế tiếp sẽ disconnect VPN "
                        "và dùng IP thật (default route), rồi lại reconnect VPN cho cycle "
                        "tiếp. Vd: --vpn-real-ip-cycle 11 → cứ 10 fake VPN thì 1 IP thật. "
                        "Dùng để reset rate-limit counter của YouTube cho IP fake tiếp theo.")
    ...
    p.add_argument("--rss-page-delay", type=int, default=5,
                   help="Delay giữa các trang RSS XML (giây, mặc định 2s). "
                        "RSS pages nhẹ hơn yt-dlp nên delay ngắn hơn.")
    p.add_argument("--transcript-delay", type=int, default=15,
                   help="Delay giữa các video trong transcribe_with_youtube loop "
                        "(giây, mặc định 15s từ 2026-06-20). Áp dụng khi yt-dlp subtitles.")
    ...
    # ===== MULTI-INSTANCE FLAGS (chạy song song nhiều tiến trình an toàn) =====
    p.add_argument("--instance-id", default=None,
                   help="ID duy nhất cho mỗi instance (mặc định: auto = "
                        "'pid<PID>_t<UNIX_TIME>'). Dùng để tách cache folder, "
                        "log file, VPN tunnel khỏi các instance khác. "
                        "Nếu chạy song song N tiến trình, truyền --instance-id "
                        "khác nhau cho mỗi cái (vd: inst1, inst2, ...).")
    p.add_argument("--cache-dir", default=None,
                   help="Folder cache riêng cho instance này. "
                        "Mặc định: <script_dir>/.cache_<instance_id>/. "
                        "KHÔNG cần truyền nếu đã dùng --instance-id (tự tách).")
    p.add_argument("--vpn-isolated", action=argparse.BooleanOptionalAction, default=True,
                   help="Mỗi instance có VPN tunnel RIÊNG (PID-based). "
                        "KHÔNG dùng 'pkill openvpn' để tránh kill nhầm tunnel "
                        "của instance khác. MẶC ĐỊNH BẬT → an toàn chạy song song "
                        "nhiều instance với --use-vpn. "
                        "Cần: sudo setcap cap_net_admin+ep /usr/sbin/openvpn "
                        "(đã có sẵn trên máy này). "
                        "Truyền --no-vpn-isolated để tắt (dùng VPNRotator gốc, có thể kill nhầm).")

    # ===== PROXY MODE (mặc định split - tận dụng IP thật cho audio) =====
    # VPN tunnel vẫn BẮT BUỘC cho metadata/transcript (fail-fast nếu không có VPN).
    # Nhưng cho phép chọn cách audio download:
    #   - split (default): attempt 1 dùng IP thật (nhanh), chỉ escalate sang VPN khi rate-limit
    #   - always: mọi audio download đều qua VPN tunnel (chậm nhưng ổn định)
    #   - never: không dùng VPN (chỉ để debug, KHÔNG khuyến nghị)
    p.add_argument("--proxy-mode", choices=["always", "split", "never"],
                   default="split",
                   help="Chiến lược dùng VPN theo giai đoạn:\n"
                        "  split  = MẶC ĐỊNH - metadata/transcript qua VPN, audio qua IP thật\n"
                        "           (KHI IP thật bị rate-limit/block khi tải audio → retry với VPN)\n"
                        "  always = tất cả audio download cũng qua VPN tunnel (chậm hơn 10-20x)\n"
                        "  never  = không dùng VPN cho audio (KHÔNG khuyến nghị)\n"
                        "\n"
                        "Khuyến nghị: GIỮ DEFAULT 'split' để tận dụng băng thông IP thật cho audio\n"
                        "(nhanh hơn 10-20x so với VPN) mà vẫn bypass Google block cho metadata.")
    return p.parse_args()
...
def process_one_channel(
    ...
    *,
    ...
    # Skip per-CHANNEL đã bị BỎ (commit trước sai):
    # Trước đây: nếu 5 file output (research JSON/CSV + pipeline_summary + 2 CSV)
    # đều có thì skip cả kênh. Hậu quả: video publish mới sau lần chạy trước
    # KHÔNG được crawl cho đến khi user truyền --force-retranscribe (ghi đè cũ).
    #
    # Cách mới: LUÔN chạy pipeline. Hàm _partition_videos_for_pipeline() sẽ tự
    # skip per-video (Bucket A = audio+json có sẵn; Bucket C = chưa có gì → download).
    # Video mới publish sẽ tự động vào Bucket C và được tải.
    #
    # Vẫn giữ log thông báo để user biết đã có output trước đó.
    if skip_existing and all_exist:
        print(f"[INFO] Channel đã có output từ lần chạy trước")
        print(f"       → Vẫn chạy pipeline để crawl video MỚI publish sau đó")
        print(f"       → Skip per-video được xử lý trong _partition_videos_for_pipeline()")
        if run_logger:
            run_logger.log(f"[INFO] Channel co output cu, van chay pipeline (skip per-video)")
    elif skip_existing and missing_files:
        print(f"[PARTIAL] Channel đã có research JSON nhưng THIẾU: {', '.join(missing_files)}")
        print(f"         → Chạy lại pipeline để tái tạo đầy đủ output")
        if run_logger:
    ...
    researcher = YouTubeResearcher(
        ...
        audio_proxy_rotator=audio_proxy_rotator,  # VPN riêng cho audio (split mode)
...
def main():
    ...
    print("=" * 80)
    print("MULTI-INSTANCE SAFE VERSION")
    print("=" * 80)
    print("Mode: yt-dlp (khong can YOUTUBE_API_KEY)")

    # === MULTI-INSTANCE: Generate INSTANCE_ID ===
    # Mỗi process có instance_id riêng để tách cache/log/tunnel.
    # Ưu tiên: --instance-id (user truyền) > auto = pid<PID>_t<UNIX_TIME>
    global INSTANCE_ID
    if args.instance_id:
        INSTANCE_ID = args.instance_id
    else:
        INSTANCE_ID = f"pid{os.getpid()}_t{int(time.time())}"
    print(f"[Multi-instance] Instance ID: {INSTANCE_ID}")
    if args.cache_dir:
        print(f"[Multi-instance] Cache dir: {args.cache_dir}")
    else:
        print(f"[Multi-instance] Cache dir: <script_dir>/.cache_{INSTANCE_ID}/")

    # === Tạo cache root sớm để các static methods dùng được ===
    cache_root = _instance_root()
    cache_root.mkdir(parents=True, exist_ok=True)

    youtube_key = "ytdlp"  # placeholder, khong su dung

    # === VPN rotator (BẮT BUỘC trong bản vpn-only) ===
    # KHÔNG có fallback IP thật. Nếu VPN không kết nối được → sys.exit(1).
    # === Mode 'always': 1 rotator cho cả metadata + audio (KHÔNG có split/never) ===
    metadata_rotator = None
    audio_rotator = None

    if not args.use_vpn:
        # Phòng trường hợp argparse custom: --use-vpn phải True trong bản này
        print("FATAL: --use-vpn phải True trong bản vpn-only. "
              "Để chạy không VPN hãy dùng bản gốc youtube_researcher_youtube_subs_multi.py "
              "và truyền --no-use-vpn.", file=sys.stderr)
        sys.exit(2)

    # VPN rotator (ProtonVPN) qua file .ovpn trong ./proton_config/
    # === Bản vpn-only: LUÔN dùng IsolatedVPNRotator (multi-instance safe) ===
    _vpn = None
    try:
        _vpn = get_isolated_vpn_rotator_from_config(
            instance_id=INSTANCE_ID,
        ...
        print(f"FATAL: Không import được vpn_rotator ({e}). "
              f"Bản này BẮT BUỘC phải có ./vpn_rotator.py. "
    ...
    if _vpn is None:
        print(
            "FATAL: Không tìm thấy file .ovpn trong ./proton_config/.\n"
            "       Bản vpn-only BẮT BUỘC phải có ProtonVPN config. "
            "Không fallback về IP thật.\n"
            "       Hướng dẫn: copy các file *.ovpn và auth.txt vào "
            "/home/hientran/sythetic_crawl_data/proton_config/",
            file=sys.stderr,
        )
        sys.exit(1)

    print(f"VPN: {len(_vpn)} ProtonVPN servers "
          f"(ISOLATED tunnel cho instance '{INSTANCE_ID}'), "
          f"rotate_every={args.vpn_rotate_every}, "
          f"strategy={args.vpn_strategy}, "
          f"real_ip_cycle={args.vpn_real_ip_cycle} "
          f"({'TẮT cycle (chỉ fake IP)' if args.vpn_real_ip_cycle <= 0 else f'BẬT cycle {args.vpn_real_ip_cycle} (= {args.vpn_real_ip_cycle - 1} fake + 1 real IP)'})")
    print(f"  • OpenVPN log: /tmp/openvpn-proton-{INSTANCE_ID}.log")
    print(f"  • OpenVPN PID file: /tmp/openvpn-proton-{INSTANCE_ID}.pid.*")
    print(f"  • KHÔNG dùng pkill → an toàn chạy song song nhiều instance")

    # === Phân bổ rotator theo --proxy-mode ===
    # VPN tunnel BẮT BUỘC cho metadata/transcript (fail-fast ở load .ovpn đã check ở trên).
    # Audio download: tùy proxy_mode (split/always/never).
    # KHÔNG verify tunnel ở đây (lazy-connect: openvpn tự connect khi fetch video đầu tiên).
    # Nếu openvpn fail khi đang fetch, code sẽ retry/rotate.
    if args.proxy_mode == "split":
        # Metadata + transcript qua VPN, audio qua IP thật (mặc định)
        metadata_rotator = _vpn
        audio_rotator = _vpn  # Nhưng chỉ dùng khi IP thật fail (xem _audio_proxy_for_dl)
        print(f"VPN: ✓ Sẵn sàng — metadata + transcript qua VPN tunnel.")
        print(f"  • PROXY-MODE=split: audio attempt 1 dùng IP thật (tận dụng băng thông),")
        print(f"    chỉ escalate sang VPN khi IP thật bị rate-limit/block")
        print(f"  • Lazy-connect: openvpn sẽ tự kết nối khi fetch video đầu tiên")
    elif args.proxy_mode == "always":
        # Tất cả audio download cũng qua VPN tunnel (chậm hơn nhưng ổn định)
        metadata_rotator = _vpn
        audio_rotator = _vpn
        print(f"VPN: ✓ Sẵn sàng — mọi request (metadata + transcript + audio) đều qua VPN tunnel.")
        print(f"  • PROXY-MODE=always: tất cả traffic qua VPN (không có audio-dùng-IP-thật)")
        print(f"  • Lazy-connect: openvpn sẽ tự kết nối khi fetch video đầu tiên")
    elif args.proxy_mode == "never":
        # Không dùng VPN cho audio (chỉ để debug)
        metadata_rotator = _vpn  # Metadata vẫn qua VPN
        audio_rotator = None    # Audio dùng IP thật hoàn toàn
        print(f"VPN: ✓ Sẵn sàng cho metadata/transcript. Audio dùng IP thật.")
        print(f"  • PROXY-MODE=never: chỉ metadata qua VPN, audio qua IP thật")
    print(f"  • Nếu openvpn fail, xem log: /tmp/openvpn-proton-{INSTANCE_ID}.log")
    # === MULTI-INSTANCE: Warn nếu output_root có vẻ bị share ===
    # (Best-effort: chỉ warn nếu path /tmp/test_* hoặc chứa 'shared')
    # Không chặn vì user có thể cố ý.
    if "shared" in str(args.output).lower() or "test" in str(args.output).lower().split("/")[-1]:
        print(f"[Multi-instance] ⚠️  --output '{args.output}' có vẻ dùng chung "
              f"giữa các instance. Cân nhắc dùng path riêng (vd: ./datasets_inst1, "
              f"./datasets_inst2).")
    ...
    # MULTI-INSTANCE: thêm millisecond + INSTANCE_ID vào log path
    # để 2 instance chạy cùng lúc KHÔNG đè log file của nhau.
    batch_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]
    log_dir = output_root / "logs"
    log_path = log_dir / f"run_{batch_timestamp}_{INSTANCE_ID}.txt"
    # Xac dinh duong dan script dang chay
    script_path = str(Path(__file__).resolve())
    run_logger = RunLogger(log_path, script_path=script_path)
    cmdline = " ".join(sys.argv) if sys.argv else "python youtube_researcher_youtube_subs_multi.py"
    ...
    # MULTI-INSTANCE: ghi instance_id vào log header
    run_logger.log(f"Instance ID   : {INSTANCE_ID} (PID={os.getpid()})")
    run_logger.log(f"Cache dir     : {_instance_root()}")
    ...
    for idx, ch_url in enumerate(channels, 1):
        ...
        try:
            result = process_one_channel(
                ...
                proxy_rotator=metadata_rotator,  # VPN cho metadata/transcript
                audio_proxy_rotator=audio_rotator,  # VPN cho audio (split mode: chỉ khi IP thật fail)
    ...
    if metadata_rotator:
        print("\n" + "=" * 80)
        print("PROXY STATS (final) - metadata_rotator")
        print("=" * 80)
        metadata_rotator.print_stats()

        # MULTI-INSTANCE: cleanup tunnel của MÌNH khi kết thúc
        # (KHÔNG ảnh hưởng tunnel của instance khác)
        if isinstance(metadata_rotator, IsolatedVPNRotator):
            print(f"\n[Multi-instance] Disconnect tunnel của instance '{INSTANCE_ID}'...")
            try:
                metadata_rotator.disconnect()
                print(f"  ✓ Tunnel instance '{INSTANCE_ID}' đã đóng.")
            except Exception as e:
                print(f"  ⚠️  Lỗi khi disconnect: {e}")
...
# /home/hientran/miniconda3/envs/crawl/bin/python3 \
#   python3 youtube_researcher_youtube_subs_multi_vpn_v2.py \
#   --channels-file /home/Downloads/sythetic_crawl_data/channels_audio/phim_hai.txt \
#   --max-results 1000 \
#   --max-fetch 5000 \
#   --video-delay 5 \
#   --skip-existing 
