...
# =============================================================================
# run_crawl_v19.sh — clone của run_crawl_v18.sh
# -----------------------------------------------------------------------------
# v19 khác v18:
#   - Sử dụng youtube_researcher_audio_subs_multi_rotator_v19.py (thay v18)
#   - Import vpn_rotator_v6 (thay v5): fix pkill tất cả openvpn khi pid=None
#   - _disconnect() không bao giờ giết tunnel của instance khác
#   - get_source_address() tự verify IP (v18.1 fix)
#   - Tunnel dead → force_rotate (không next() reconnect vô ích)
...
CHANNELS_FILE=/home/hientran/sythetic_crawl_data/channels_audio/channels_review_4.txt
...
#   Set 0 = tắt.
TRANSCRIPT_ROTATE_EVERY="${TRANSCRIPT_ROTATE_EVERY:-20}"
...
echo "[run_crawl] starting crawler (v19: FIX pkill all + tunnel dead force_rotate + v18 fixes)..."
...
exec "$PY" \
  "$SCRIPT_DIR/youtube_researcher_audio_subs_multi_rotator_v19.py" \
  "${CLI_ARGS[@]}"